...
#include "bundling/BundleListIntMap.h"
...
BundleListIntMap::iterator iter;
BundleMappings::iterator map_iter;

BundleListIntMap *l1, *l2, *l3;
...
DECLARE_TEST(Init) {
    ...
    l1 = new BundleListIntMap("list1");
    l2 = new BundleListIntMap("list2");
    l3 = new BundleListIntMap("list3");
...
DECLARE_TEST(ContainsAndErase) {
    ...
    CHECK(! l1->contains((Bundle*)NULL));
    CHECK(! l1->erase((Bundle*)NULL));
    CHECK(!l1->empty());
    CHECK_EQUAL(l1->size(), COUNT - 3);
...
DECLARE_TEST(MultipleLists) {
         ...
         ++map_iter)
    {
        ...
        if ( ( l = dynamic_cast<BundleListIntMap *>(lb) ) )
        {
            SPV vpos = bmap->position();
            BundleListIntMap::iterator* bitr = (BundleListIntMap::iterator*)vpos.get();
    ...
    while ( ! done )
    {
        ...
        while ( map_iter != b->mappings()->end() ) 
        {
            ...
            if ( BundleDaemon::instance()->all_bundles() != lb )
            {
                l = (BundleListIntMap*) lb;
