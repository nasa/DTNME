...
#include "bundling/BundleListStrMap.h"
...
BundleListStrMap::iterator iter;
BundleMappings::iterator map_iter;

BundleListStrMap *l1, *l2, *l3;
...
DECLARE_TEST(Init) {
    ...
    l1 = new BundleListStrMap("list1");
    l2 = new BundleListStrMap("list2");
    l3 = new BundleListStrMap("list3");
...
DECLARE_TEST(MultipleLists) {
         ...
         ++map_iter)
    {
        ...
        if ( ( l = dynamic_cast<BundleListStrMap *>(lb) ) )
        {
            SPV vpos = bmap->position();
            BundleListStrMap::iterator* bitr = (BundleListStrMap::iterator*)vpos.get();
        ...
        while ( ! done )
        {
            ...
            while ( map_iter != b->mappings()->end() ) 
            {
                ...
                lb = (BundleListBase*) bmap->list();
                 ...
                 // skip the all_bundles_list entry
                if ( typeid(BundleListStrMap) == typeid(*lb) )
                {
                    l = (BundleListStrMap*) lb;
