...
int count = 15;
char dest_eid_str[DTPC_MAX_ENDPOINT_ID-1] = "";
int user_exit = 0;
...
main(int argc, const char** argv)
{
    ...
    dest_eid.uri[sizeof(dest_eid.uri)-1] = '\0';
...
doOptions(int argc, const char **argv)
{
    ...
    while ( (c=getopt(argc, (char **) argv, "A:B:hc:i:d:p:t:")) !=EOF ) {
        ...
        case 'i':
            ...
            break;
        case 'd':
            strncpy(dest_eid_str, optarg, sizeof(dest_eid_str)-1);
            dest_eid_str[sizeof(dest_eid_str)-1] = '\0';
            break;
        case 'p':
            profile = atoi(optarg);
            profile_set = 1;
    ...
    if ((optind < argc) && (strlen(dest_eid_str) == 0)) {
        strncpy(dest_eid_str, argv[optind++], sizeof(dest_eid_str)-1);
        dest_eid_str[sizeof(dest_eid_str)-1] = '\0';
    }
