...
// helper thread to asynchronously delete a file
...
usage()
{
    ...
    fprintf(stderr, " -v verbose frequency <# bundles>\n");
 ...
 */
int
handle_file_transfer(dtn_bundle_payload_t payload, uint64_t *size, uint32_t *which)
{
    *size = payload.size_of_payload;
    ...
    if (*size != (uint64_t) fileinfo.st_size) {
        printf("Error - expected tmp payload file of size: %zu but actual size is: %zu\n",
...
/* ----------------------------------------------------------------------- */
/* DTN daemon has released its DB copy of the payload to the app. 
 * The app is responsible for deleting the file
 */
int
handle_released_db_file_transfer(dtn_bundle_payload_t payload, uint64_t *size, uint32_t *which)
{
    *size = payload.size_of_payload;
    ...
    // Try to open the file
    int tempdes = open(payload.filename.filename_val, O_RDONLY);
    if ( tempdes < 0 ) {
        fprintf(stderr, "While opening the released DB file for reading '%s': %s\n",
        payload.filename.filename_val, strerror(errno));
        return 0;
    }

    if (fstat(tempdes, &fileinfo) != 0) {
        fprintf(stderr, "While stat'ing the released DB file '%s': %s\n",
        payload.filename.filename_val, strerror(errno));
        close(tempdes);
        return -1;
    }

    if ((size_t)fileinfo.st_size > sizeof(*which)) {
        if (read(tempdes, which, sizeof(*which)) != sizeof(*which)) {
            fprintf(stderr, "While reading bundle number from released DB file '%s': %s\n",
            payload.filename.filename_val, strerror(errno));
            close(tempdes);
            return -1;
        }
...
main(int argc, char** argv)
{
    ...
    printf("%s starting up -- waiting for %u bundles\n", argv[0], count);

    // open the ipc handle
    if (verbose) printf("opening connection to dtnme...\n");
    ...
    if (verbose) printf("opened connection to dtnme...\n");
    ...
    while (i <= count) {
        ...
        if (!promiscuous) {
            ...
            if ((verbose > 0) && (bundles_received % verbose == 0)) {
                printf("bundle %d received successfully: id %s,%zu.%zu  payload len: %"PRIu64"\n",
                       i,
                       spec.source.uri,
            ...
            if (dots) {
                printf(".");
                fflush(stdout);
            } else if ((verbose > 0) && (bundles_received % verbose == 0)) {
...
bail:
    ...
    if (bundles_received == 0) {
        printf("Exiting with zero bundles received\n");
    } else {
