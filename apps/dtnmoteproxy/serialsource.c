...
static void message(serial_source_ptr src, serial_source_msg msg)
{
  if (src->message)
    src->message(msg);
}
...
static int buggyread(serial_source_ptr src, void *buffer, int n)
{
  fd_set fds;
  int cnt;
...
serial_source_ptr open_serial_source(const char *device, int baud_rate,
				 int non_blocking,
				 void (*message)(serial_source_msg problem))
 ...
 */
{
  ...
  if (tcflush(fd, TCIFLUSH) >= 0 &&
    ...
    {
      serial_source_ptr src = malloc(sizeof(struct serial_source));

      if (src)
	{
	  memset(src, 0, sizeof(struct serial_source));
	  src->fd = fd;
...
int serial_source_fd(serial_source_ptr src)
...
int close_serial_source(serial_source_ptr src)
...
static int source_wait(serial_source_ptr src, struct timeval *deadline)
...
static int source_write(serial_source_ptr src, const void *buffer, int count)
{
  int actual = 0;
...
static void push_protocol_packet(serial_source_ptr src,
...
static struct packet_list *pop_protocol_packet(serial_source_ptr src, uint8_t type)
{
...
static bool packet_available(serial_source_ptr src, uint8_t type)
{
  return src->recv.queue[type] != NULL;
}

int serial_source_empty(serial_source_ptr src)
...
static int read_byte(serial_source_ptr src)
...
static void process_packet(serial_source_ptr src, uint8_t *packet, int len);
static int write_framed_packet(serial_source_ptr src,
			       uint8_t packet_type, uint8_t first_byte,
			       const uint8_t *packet, int count);

static void read_and_process(serial_source_ptr src)
...
void *read_serial_packet(serial_source_ptr src, int *len)
...
static void escape_add(serial_source_ptr src, uint8_t b)
{
  src->send.escaped[src->send.escapeptr++] = b;
}

static int init_escaper(serial_source_ptr src, int count)
{
...
static void terminate_escaper(serial_source_ptr src)
{
  escape_add(src, SYNC_BYTE);
}

static void escape_byte(serial_source_ptr src, uint8_t b)
{
...
static void free_escaper(serial_source_ptr src)
{
...
int write_serial_packet(serial_source_ptr src, const void *packet, int len)
