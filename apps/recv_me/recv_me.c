...
int buildfilename(char* template, char* newfilename, int counter )
{
    ...
    snprintf(formatstring, sizeof(formatstring), "%%0%dd", templatelen);
    snprintf(counterasstring, sizeof(counterasstring), formatstring, counter);
                     ...
                     uint64_t* total_bytes, int counter)
{
    ...
    size_t block_size = (payload.size_of_payload < BLOCKSIZE) ? payload.size_of_payload : BLOCKSIZE;
    ...
    if (rename(filename, currentfile) == 0) {
        ...
        // Duplicate the file
        while ( (bytesread = read(tempdes, block, block_size)) > 0 ) {
            if (-1 == write(destdes, block, bytesread)) {
        ...
        if (bytes_copied != payload.size_of_payload) {
            fprintf(stderr, "Incorrect file size - expected: %zu got: %zu - not deleting tmp file: %s\n",
                    payload.size_of_payload, bytes_copied, payload.filename.filename_val);
        } else {
    ...
    printf("%zu byte file from [%s]: transit time=%d ms, written to '%s'\n",
           fileinfo.st_size, spec.source.uri, 0, currentfile);        
 ...
 */
int
get_payload_size_from_file(dtn_bundle_payload_t payload,
                           uint64_t* total_bytes)
{
    ...
    tempdes = open(payload.filename.filename_val, O_RDONLY);
    if ( tempdes < 0 ) {
        fprintf(stderr, "Error opening the temporary file for reading '%s': %s\n",
                payload.filename.filename_val, strerror(errno));
        return 0;
    }
...
main(int argc, char** argv)
{
    ...
    if (count == 0) { 
        printf("recv_me (pid %d) starting up\n", getpid());
    } else {
        printf("recv_me (pid %d) starting up -- count %u\n", getpid(), count);
    }
    ...
    while ((count == 0) || (bundle_count < count)) {
        ...
        if (do_stats) {
            ...
            if ( 0 == bundle_count ) {
                first_pkt_time = now;
                last_rpt = now;
                first_bundle_size = payload.size_of_payload;
            }
            ...
            if ((0 == bundle_count) || (now > last_rpt)) {
                ...
                elapsed = now - first_pkt_time;
                if (elapsed > 0) {
                    bundles_per_sec = (bundle_count-1) / elapsed;
                    Mbps = ((total_bytes - first_bundle_size)  * 8.0) / 1000000.0 / elapsed;
                }
        ...
        if (bundletype == DTN_PAYLOAD_FILE)
        {
            ...
            bundle_count++;
            continue;
        } else if (payload.location == DTN_PAYLOAD_FILE) {
            get_payload_size_from_file(payload, &total_bytes);
        } else {
    ...
    if (do_stats) {
        ...
        printf("\nFinal rcvd bundle #%d 1sec: %d -  elapsed: %lds  Avg Bundles/sec: %d  Data Mbps: %.3f\n", 
               bundle_count, bundles_this_sec, elapsed, bundles_per_sec, Mbps);
        printf("(NOTE: first bundle and its size was not included in the rate calculations and elapsed resolution is not sub-second)\n\n");
        fflush(stdout);
    }

    printf("recv_me (pid %d) exiting: %d bundles received, %"PRIu64" total bytes\n\n",
