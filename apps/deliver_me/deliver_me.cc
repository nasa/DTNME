...
{
  bool valid = false;
  std::vector<std::string> valid_types = {"put.request","transaction.list","transaction.info","exit"};
  ...
  for(u_int i=0;i<valid_types.size();i++){
    if(i == 0){
      ...
      fprintf(stdout,"%s <transaction_id>\n",valid_types[i].c_str());
    }
  }
  fprintf(stdout,"|-------------|\n");
  ...
  if(input.size()>0){
    ...
    for(u_int i=0;i<valid_types.size();i++){
      if(command == valid_types[i]){
        ...
        if((i == 0 && args.size() == 5) || args.size() == 2 || args.size() == 1){
          valid = true;
        }
      }
    }
...
{
  std::vector<std::string> valid_types = {"put.request","transaction.list","transaction.info"};
  while(!handle_->should_shutdown_){
    if(!handle_->primitive_buffer_.empty()){
      if(handle_->primitive_buffer_.front()[0] == valid_types[0]){
          ...
          u_int sequence_number = sequence_number_;
          ...
          while(!storage_.eof() && handle_->file_segment_size_ > 0){
            ...
            if (bytes_read < handle_->file_segment_size_) {
                if (!storage_.eof()) {
                    // error ...
...
Deliver_Me::Transaction_Worker::run()
{
  ...
  while(should_work_){
    ...
    if(source_entity_id != handle_->local_entity_id_){
      ...
      if( (offs >= file_size) && !eof_.set){


        //XXX TODO: what if bundles are received out of order??? 
        //          need to track if all segments written before closing the file;
      ...
      /*
      //open file and generate bundles
      if(!storage_.is_open()){
        storage_.open(meta_buffer_.front().source_filename, std::fstream::in | std::fstream::app | std::fstream::binary);
        if(!storage_){
          handle_->log_lock_.lock("Deliver_Me::Transaction_Worker::run");
          handle_->log_buffer_.push_back("Error: Could not open File Path");
          handle_->log_lock_.unlock();
        }
        char* buffer[handle_->segment_size_];
        
        int loop=0;
        while(!storage_.eof()){
          storage_.read(buffer,handle_->segment_size_);
          //Concat fixed_header+segment_offset+buffer

          dtn_bundle_payload_t payload;
          dtn_set_payload(&payload, DTN_PAYLOAD_MEM,pdu,pdu_length+9);
          
          //create bundle payload
          handle_->transmit_lock_.lock("Deliver_Me::Transaction_Worker::run");
          handle_->transmit_buffer_.push_back(spec,payload);
          handle_->transmit_lock_.unlock();
          loop++;
        }        

      } else {
        //Ooops Storage was already open???
      }
      */
    }
  }
