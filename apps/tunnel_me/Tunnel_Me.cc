...
namespace oasys {
    template <> tunnel_me::Tunnel_Me* oasys::Singleton<tunnel_me::Tunnel_Me>::instance_ = 0;
}
...
//----------------------------------------------------------------------
Tunnel_Me::Tunnel_Me()
    : App("Tunnel_Me", "tunnel_me"),
      send_lock_("/tunnel_me", oasys::Mutex::TYPE_RECURSIVE, true),
      listen_(false),
      custody_(false),
      expiration_(600),
      tcp_(true),
...
Tunnel_Me::fill_options()
{
    ...
    opts_.addopt(
        new oasys::UIntOpt('V', "bp_verson", &bp_version_, "<value>",
                           "Bundle Protocol Version (6, 7, or 0 = server default)"));

}

//----------------------------------------------------------------------
void
Tunnel_Me::validate_options(int argc, char* const argv[], int remainder)
{
    if (udp_) {
        tcp_ = false;
    }
    ...
    if ((bp_version_ != 0) && (bp_version_ != 6) && (bp_version_ != 7)) {
        fprintf(stderr, "invalid Bundle Protcol Version: '%u'\n", bp_version_);
        print_usage_and_exit();
    }
...
//----------------------------------------------------------------------
void
Tunnel_Me::load_dest_eid_table(char* filename)
{
    FILE* ifile;
    char line[512];
...
//----------------------------------------------------------------------
void
Tunnel_Me::get_dest_eid(in_addr_t remote_addr, dtn_endpoint_id_t* dest_eid)
{
...
Tunnel_Me::init_registration()
{
    ...
    if (listen_) {
        demux.appendf("tunnel_me");
    } else {
        demux.appendf("tunnel_me?dest_eid=%s&tunnel=%s-%s:%u-%s:%u",
                      dest_eid_.uri,
                      tcp_ ? "tcp" : "udp",
                      intoa(local_addr_),
                      local_port_,
...
//----------------------------------------------------------------------
int
Tunnel_Me::send_bundle(dtn::APIBundle* bundle, dtn_endpoint_id_t* dest_eid)
{
    // lock to coordinate access to the send_handle_ from multiple
    // client threads
    oasys::ScopeLock l(&send_lock_, "Tunnel_Me::send_bundle");
    
    dtn_bundle_spec_t spec;
    memset(&spec, 0, sizeof(spec));

    spec.bp_version = bp_version_;
...
//----------------------------------------------------------------------
int
Tunnel_Me::handle_bundle(dtn_bundle_spec_t* spec,
                         dtn_bundle_payload_t* payload)
{
...
//----------------------------------------------------------------------
void
Tunnel_Me::output_throughput_stats()
{
...
//----------------------------------------------------------------------
int
Tunnel_Me::main(int argc, char* argv[])
{
    init_app(argc, argv);

    log_notice("Tunnel_Me starting up...");
    ...
    log_debug("Tunnel_Me starting receive loop...");
...
//----------------------------------------------------------------------
Tunnel_Me::StatusThread::StatusThread()
    : Thread("UDPTunnel::StatusThread", DELETE_ON_EXIT),
      Logger("UDPTunnel::StatusThread", "/tunnel_me/statusthread") 
{
    start();
}
...
//----------------------------------------------------------------------
void
Tunnel_Me::StatusThread::run()
{
    Tunnel_Me* tunnel = Tunnel_Me::instance();
...
{
    tunnel_me::Tunnel_Me::create();
    tunnel_me::Tunnel_Me::instance()->main(argc, argv);
}
