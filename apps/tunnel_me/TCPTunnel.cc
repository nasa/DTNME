...
//----------------------------------------------------------------------
TCPTunnel::TCPTunnel()
    : IPTunnel("TCPTunnel", "/tunnel_me/tcp"),
...
{
    transparent_ = Tunnel_Me::instance()->transparent();
...
TCPTunnel::Listener::force_close_socket(int fd)
{
    ...
    if (-1 == ::write(fd, "", 1)) {
        log_err("Error writing to socket; %s", strerror(errno));
    }
    ::close(fd);
}
...
TCPTunnel::Listener::accepted(int fd, in_addr_t addr, u_int16_t port)
{
    ...
    dtn_copy_eid(&dest_eid, Tunnel_Me::instance()->dest_eid());
    Tunnel_Me::instance()->get_dest_eid(remote_addr_, &dest_eid);
    if (dest_eid.uri[0] == '\0') {
...
TCPTunnel::Connection::Connection(TCPTunnel* t, dtn_endpoint_id_t* dest_eid,
    ...
    : Thread("TCPTunnel::Connection", Thread::DELETE_ON_EXIT),
      Logger("TCPTunnel::Connection", "/tunnel_me/tcp/conn"),
      tcptun_(t),
      sock_("/tunnel_me/tcp/conn/sock"),
      queue_("/tunnel_me/tcp/conn"),
      next_seqno_(0),
...
TCPTunnel::Connection::Connection(TCPTunnel* t, dtn_endpoint_id_t* dest_eid,
    ...
    : Thread("TCPTunnel::Connection", Thread::DELETE_ON_EXIT),
      Logger("TCPTunnel::Connection", "/tunnel_me/tcp/conn"),
      tcptun_(t),
      sock_(fd, client_addr, client_port, "/tunnel_me/tcp/conn/sock"),
      queue_("/tunnel_me/tcp/conn"),
      next_seqno_(0),
      client_addr_(client_addr),
      client_port_(client_port),
    ...
    while (1) {
        ...
        if (sock_poll->revents != 0) {
            ...
            if (payload_todo != 0) {
                ...
                if (ret == 0) {
                    Tunnel_Me::BundleHeader* hdrp =
                        (Tunnel_Me::BundleHeader*)b_xmit->payload_.buf();
                    hdrp->eof_ = 1;
                    sock_eof = true;
                }
            }
        ...
        if (msg_poll->revents != 0) {
            ...
            Tunnel_Me::BundleHeader* recv_hdr =
                (Tunnel_Me::BundleHeader*)b_recv->payload_.buf();
...
{
    Tunnel_Me::BundleHeader* hdr =
        (Tunnel_Me::BundleHeader*)bundle->payload_.buf();
