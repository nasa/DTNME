...
main(int argc, char** argv)
{
	...
	memset(payload_buf + sizeof(uint32_t), ((unsigned char)i & 0xff), arg_bundlesize);
        ...
        if (verbose) fprintf(stdout, "bundle sent successfully: id %s,%zu.%zu \n",
...
void print_usage()
{
    ...
    fprintf(stderr, " -V <int> Bundle Protocol Version (6, 7, or 0 = server default)\n");
    exit(1);
}
...
void parse_options(int argc, char**argv)
{
    ...
    while (!done)
    {
        c = getopt(argc, argv, "A:B:vhs:d:b:Hr:e:P:n:woDXFRcC1NWi:z:E:M:O:S:V:");
        switch (c)
        {
        case 'A':
            api_IP_set = 1;
