...
short api_port = 5010;
int bp_version = 0; // valid options are 6, 7 or 0

void
usage()
{
    fprintf(stderr, "usage: %s [-A api_IP] [-B api_port] [-V BP version] [-e expiration] [-w waittime] eid\n",
            progname);
    fprintf(stderr, "               [-s source EID] [-r reply to EID] [-d destination EID]\n");
    exit(1);
}
...
doOptions(int argc, const char **argv)
{
    ...
    while ( (c=getopt(argc, (char **) argv, "A:B:he:d:s:r:w:V:")) !=EOF ) {
        switch (c) {
        case 'A':
            api_IP_set = 1;
            api_IP = optarg;
