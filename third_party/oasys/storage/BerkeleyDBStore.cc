...
#undef SET_DBENV_OPTION
    ...
    err = dbenv_->open(dbenv_, dbdir.c_str(), dbenv_opts, 0777 /* permissions based on process umask */);
    ...
    if (cfg.db_lockdetect_ != 0) {
        deadlock_timer_ = std::make_shared<DeadlockTimer>(logpath_, dbenv_, cfg.db_lockdetect_);
        deadlock_timer_->set_sptr(deadlock_timer_);
        deadlock_timer_->reschedule();
    } else {
        deadlock_timer_ = nullptr;
    }
...
{
    if (sptr_ != nullptr) {
        schedule_in(frequency_, sptr_);
    }
}

//----------------------------------------------------------------------------
void
BerkeleyDBStore::DeadlockTimer::cancel()
{
    if (sptr_ != nullptr) {
