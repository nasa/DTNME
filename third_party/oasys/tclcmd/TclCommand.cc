    ...
    if (command->do_builtins_) 
    {
        if (objc >= 2) {
            ...
            if (strcmp(cmd, "set") == 0) {
                ...
                // preventing startup abort on minor errors
                //return TCL_OK;
            }
        }
    }
...
TclCommand::cmd_set(int objc, Tcl_Obj** objv, Tcl_Interp* interp)
{
    ...
    if (itr == bindings_.end()) {
        ...
        if (Log::initialized()) {
            log_err("Warning: \"%s set\" binding for %s does not exist", 
                      Tcl_GetStringFromObj(objv[0], 0), var);
        }
    ...
    if (val) {
        if (opt->set(val, val_len) != 0) {
            ...
            if (Log::initialized()) {
                log_err("Error: \"%s set %s\" has invalid value: %s", 
                          Tcl_GetStringFromObj(objv[0], 0), var, val);
            }
            //dzdebug return TCL_ERROR;
            return TCL_OK;
        ...
        if (validate(var, val, opt) != 0) {
            if (Log::initialized()) {
                log_err("Error: \"%s set %s %s\" failed validation", 
                          Tcl_GetStringFromObj(objv[0], 0), var, val);
            }
            //dzdebug return TCL_ERROR;
            return TCL_OK;
