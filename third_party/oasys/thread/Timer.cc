...
template <> SharedTimerSystem* Singleton<SharedTimerSystem>::instance_ = nullptr;
...
//----------------------------------------------------------------------
void
SharedTimerSystem::schedule_in(time_t milliseconds, SPtr_Timer timer)
{
    if (timer == nullptr) return;
...
//----------------------------------------------------------------------
bool
SharedTimerSystem::cancel_timer(SPtr_Timer timer)
{
    if (timer == nullptr) return false;

    if (!should_stop_ && !pause_processing_) {
        ScopeLock cl(cancel_lock_, "SharedTimerSystem::cancel_timer");
...
//----------------------------------------------------------------------
size_t
SharedTimerSystem::num_cancelled_timers()
{
    ScopeLock cl(cancel_lock_, "SharedTimerSystem::num_cancelled_timers");
...
//----------------------------------------------------------------------
void
SharedTimer::schedule_in(time_t milliseconds, SPtr_Timer sptr)
{
...
//----------------------------------------------------------------------
bool
SharedTimer::cancel_timer(SPtr_Timer sptr)
{
    return SharedTimerSystem::instance()->cancel_timer(sptr);
}
