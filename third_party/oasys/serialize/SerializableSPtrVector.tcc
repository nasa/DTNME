...
void
SerializableSPtrVector<_Type>::serialize(SerializeAction* a)
{
    ...
    if (a->action_code() == oasys::Serialize::UNMARSHAL) {
        ...
        for (size_t i = 0; i < sz; ++i) {
            this->push_back(std::make_shared<_Type>(builder));
            a->process("element", this->back().get());
        }
        ASSERT(this->size() == sz);
    } else {
        // if we're marshalling (or sizing), then just call process on
        // all elements in the vector
        typename std::vector<std::shared_ptr<_Type>>::iterator i;
        for (i = this->begin(); i != this->end(); ++i) {
            a->process("element", (*i).get());
        }
    }
}
