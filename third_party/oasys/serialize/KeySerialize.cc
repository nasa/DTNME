                    ...
                    BufferCarrier<u_char>* carrier)
{
    ...
    memcpy(buf_->eb_end(), carrier->buf(), carrier->len());
                    ...
                    std::string* s)
{
    ...
    memcpy(buf_->eb_end(), s->c_str(), s->size());
...
KeyMarshal::process_int(u_int32_t i, u_int32_t size, const char* format)
{
    ...
    int cc = snprintf(buf_->eb_end(), size + 1, format, i);
    ASSERT(cc == (int)size);
...
KeyMarshal::border()
{
    ...
    memcpy(buf_->eb_end(), border_, border_len);
