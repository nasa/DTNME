...
FileIOClient::copy_contents(FileIOClient* dest, size_t len)
{
    ...
    while (1) {
        if (origlen == 0) {
            ...
            n = std::min(len, buflen);
        }

        cc = ::read(fd_, buf, buflen);

//dzdebug        cc = read(buf, n);

        if (cc < 0) {
            log_err("copy_contents: error reading %zd bytes: %s",
                    n, strerror(errno));
            free(buf);
            return -1;
        }
        ...
        if (cc == 0) {
            if (origlen != 0 && len != 0) {
                log_err("copy_contents: file %s too short (expected %zu bytes)",
                        path_.c_str(), origlen);
                free(buf);
                return -1;
            }
...
//dzdebug        if (dest->writeall(buf, cc) != cc) {
        if (cc != ::write(dest->fd(), buf, cc)) {
            log_err("copy_contents: error writing %zd bytes: %s",
                    cc, strerror(errno));
            free(buf);
            return -1;
        }
