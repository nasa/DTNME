...
Log::add_rotate_handler(int sig)
{
    ...
    SharedTimerSystem::instance()->add_sighandler(sig, rotate_handler);
}
...
Log::add_reparse_handler(int sig)
{
    ...
    SharedTimerSystem::instance()->add_sighandler(sig, reparse_handler);
}
...
Log::set_loglevel(log_level_t defaultlvl)
{
    ...
    logf("/log", LOG_ALWAYS, "default log level set to %s",
         level2str(default_threshold_));
                ...
                const char* classname, const void* obj) const
{
    ...
    if (output_flags_ & OUTPUT_TIME) {
        Time t;
        t.get_time();
        len = snprintf(ptr, buflen, "%lu.%06lu ", t.sec_, t.usec_);
