...
UInt64Opt::set(const char* val, size_t len)
{
    ...
    if (endptr != (val + len))
    {
        ...
        switch (*endptr) {
        case 'B': case 'b': break;
        case 'K': case 'k': newval *= 1000; break;
        case 'M': case 'm': newval *= 1000000; break;
        case 'G': case 'g': newval *= 1000000000; break;
        case 'T': case 't': newval *= 1000000000000; break;
        default:
            return -1;
        }
    }
...
{
    u_int64_t mag_val;
    u_int64_t sval = *(u_int64_t*)valp_;

    buf->appendf("%lu", sval);

    if (sval >= 1000000000000) {
        mag_val = sval / 1000000000000;
        if ((mag_val * 1000000000000) == sval) {
            buf->appendf("   (%zuT)", mag_val);
        }
    } else if (sval >= 1000000000) {
        mag_val = sval / 1000000000;
        if ((mag_val * 1000000000) == sval) {
            buf->appendf("   (%zuG)", mag_val);
        }
    } else if (sval >= 1000000) {
        mag_val = sval / 1000000;
        if ((mag_val * 1000000) == sval) {
            buf->appendf("   (%zuM)", mag_val);
        }
    } else if (sval >= 1000) {
        mag_val = sval / 1000;
        if ((mag_val * 1000) == sval) {
            buf->appendf("   (%zuK)", mag_val);
        }
    }
}
...
//----------------------------------------------------------------------
Octal16Opt::Octal16Opt(const char* opt, u_int16_t* valp,
                     const char* valdesc, const char* desc, bool* setp)
    : Opt(0, opt, valp, setp, true, valdesc, desc)
{
}
...
//----------------------------------------------------------------------
Octal16Opt::Octal16Opt(char shortopt, const char* longopt, u_int16_t* valp,
                     const char* valdesc, const char* desc, bool* setp)
    : Opt(shortopt, longopt, valp, setp, true, valdesc, desc)
{
}
...
//----------------------------------------------------------------------
int
Octal16Opt::set(const char* val, size_t len)
{
    u_int newval;
    char* endptr = 0;
...
//----------------------------------------------------------------------
void
Octal16Opt::get(StringBuffer* buf)
{
    buf->appendf("0%o", *(u_int16_t*)valp_);
}

//----------------------------------------------------------------------
Octal8Opt::Octal8Opt(const char* opt, u_int8_t* valp,
                   const char* valdesc, const char* desc, bool* setp)
    : Opt(0, opt, valp, setp, true, valdesc, desc)
{
}
...
//----------------------------------------------------------------------
Octal8Opt::Octal8Opt(char shortopt, const char* longopt, u_int8_t* valp,
                   const char* valdesc, const char* desc, bool* setp)
    : Opt(shortopt, longopt, valp, setp, true, valdesc, desc)
{
}
...
//----------------------------------------------------------------------
int
Octal8Opt::set(const char* val, size_t len)
{
    u_int newval;
    char* endptr = 0;
...
//----------------------------------------------------------------------
void
Octal8Opt::get(StringBuffer* buf)
{
    buf->appendf("0%o", *(u_int8_t*)valp_);
}

//----------------------------------------------------------------------
SizeOpt::SizeOpt(const char* opt, size_t* valp,
                 const char* valdesc, const char* desc, bool* setp)
    : Opt(0, opt, valp, setp, true, valdesc, desc)
{
}
...
//----------------------------------------------------------------------
SizeOpt::SizeOpt(char shortopt, const char* longopt, size_t* valp,
                 const char* valdesc, const char* desc, bool* setp)
    : Opt(shortopt, longopt, valp, setp, true, valdesc, desc)
{
}
...
{
    buf->appendf("%" PRIu64, (*(u_int64_t*)valp_));

    u_int64_t mag_val;
    u_int64_t sval = *(u_int64_t*)valp_;

    buf->appendf("%" PRIu64, sval);

    if (sval >= 1000000000) {
        mag_val = sval / 1000000000;
        if ((mag_val * 1000000000) == sval) {
            buf->appendf("   (%" PRIu64 " Gbps)", mag_val);
        }
    } else if (sval >= 1000000) {
        mag_val = sval / 1000000;
        if ((mag_val * 1000000) == sval) {
            buf->appendf("   (%" PRIu64 " Mbps)", mag_val);
        }
    } else if (sval >= 1000) {
        mag_val = sval / 1000;
        if ((mag_val * 1000) == sval) {
            buf->appendf("   (%" PRIu64 " Kbps)", mag_val);
        }
    } else {
        buf->append(" bps");
    }
