...
struct Time {
    ...
    uint64_t in_microseconds() const;
    
    //! @return Time in milliseconds
    uint64_t in_milliseconds() const;

    //! @return Milliseconds elapsed between this timer and the
    // current time.
    uint64_t elapsed_ms() const;

    //! @return Microseconds elapsed between this timer and the
    // current time.
    ...
    void add_seconds(uint64_t secs);
    void add_milliseconds(uint64_t msecs);
    void add_microseconds(uint64_t usecs);
    //! @}
