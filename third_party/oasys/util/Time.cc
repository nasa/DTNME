...
Time::Time(double d)
{
    sec_  = static_cast<uint64_t>(floor(d));
    ...
    usec_ = static_cast<uint64_t>((d - floor(d) + 0.0000001) * 1000000);
...
Time::get_time() 
{
    ...
    // Get microseconds
    ft /= 10; 
    usec_ = static_cast<uint64_t>(ft % 1000000);

    // Get seconds
    ft /= 1000000;
    sec_ = static_cast<uint64_t>(ft);

#else
    struct timeval tv;

    gettimeofday(&tv, 0);
    sec_  = static_cast<uint64_t>(tv.tv_sec);
    usec_ = static_cast<uint64_t>(tv.tv_usec);
...
//----------------------------------------------------------------------
void
Time::add_seconds(uint64_t secs)
{
...
//----------------------------------------------------------------------
void
Time::add_milliseconds(uint64_t msecs)
{
    sec_  += msecs / 1000;
...
//----------------------------------------------------------------------
void
Time::add_microseconds(uint64_t usecs)
{
    sec_  += usecs / 1000000;
    usec_ += usecs % 1000000;
    cleanup();
...
TimeScope::~TimeScope()
{
    logf(path_, level_, "%s took %lu milliseconds", 
