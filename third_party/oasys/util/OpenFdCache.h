...
struct OpenFdCacheClose {
    static void close(int fd) {
        //log_always_p("/OpenFdCacheClose", "closed %d", fd);
        IO::close(fd);
    }
};
...
public:
    ...
    /*!
     * Attempt to evict a file that is not in use to 
     */
    bool try_to_evict_one_file() 
    {
        ScopeLock l(&lock_, "OpenFdCache::try_to_evict_one_file");
