...
{
    mutable_remote_eid() = sptr_remote_eid;
...
DtpcDataPduCollector::send_ack(DtpcProtocolDataUnit* pdu, BundleRef& rcvd_bref)
{
    ...
    if (pdu->remote_eid()->is_ipn_scheme()) {
        ...
        size_t service = pdu->remote_eid()->service_num();

        if (service == DtpcDaemon::params_.ipn_transmit_service_number) {
            service = DtpcDaemon::params_.ipn_receive_service_number;
        }
        bref->mutable_dest() = BD_MAKE_EID_IPN(pdu->remote_eid()->node_num(), service);
    } else {
        bref->mutable_dest() = pdu->remote_eid();
    }
    // default to null eid   bref->mutable_replyto() = rcvd_bref->replyto();
    // default to null eid   bref->mutable_custodian() = BD_MAKE_EID_NULL();
