...
ExternalRouterServerIF::encode_restage_cl_entry_v0(CborEncoder& entryEncoder, SPtr_RestageCLStatus& sptr_clstatus)
{
    ...
    // link name
    err = cbor_encode_text_string(&entryEncoder, sptr_clstatus->restage_link_name().c_str(), sptr_clstatus->restage_link_name().length());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // storage path
    err = cbor_encode_text_string(&entryEncoder, sptr_clstatus->storage_path().c_str(), sptr_clstatus->storage_path().length());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // mount point flag
    err = cbor_encode_boolean(&entryEncoder, sptr_clstatus->mount_point());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    //validated mount point 
    err = cbor_encode_text_string(&entryEncoder, sptr_clstatus->validated_mount_pt().c_str(), sptr_clstatus->validated_mount_pt().length());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // mount point validated flag
    err = cbor_encode_boolean(&entryEncoder, sptr_clstatus->mount_pt_validated());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // storage path exists flag
    err = cbor_encode_boolean(&entryEncoder, sptr_clstatus->storage_path_exists());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // part of pool flag
    err = cbor_encode_boolean(&entryEncoder, sptr_clstatus->part_of_pool());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // Total volume space
    err = cbor_encode_uint(&entryEncoder, sptr_clstatus->vol_total_space());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // Available volume space
    err = cbor_encode_uint(&entryEncoder, sptr_clstatus->vol_space_available());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // Max disk quota
    err = cbor_encode_uint(&entryEncoder, sptr_clstatus->disk_quota());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // Disk quota in use
    err = cbor_encode_uint(&entryEncoder, sptr_clstatus->disk_quota_in_use());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // Num files restaged
    err = cbor_encode_uint(&entryEncoder, sptr_clstatus->disk_num_files());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // Num days retention
    err = cbor_encode_uint(&entryEncoder, sptr_clstatus->days_retention());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // Expire bundles flag
    err = cbor_encode_boolean(&entryEncoder, sptr_clstatus->expire_bundles());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // Time to live override when reloading bundles
    err = cbor_encode_uint(&entryEncoder, sptr_clstatus->ttl_override());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // Auto-reload interval (seconds
    err = cbor_encode_uint(&entryEncoder, sptr_clstatus->auto_reload_interval());
    CBORUTIL_CHECK_CBOR_ENCODE_ERROR_RETURN

    // current state of the restage CL
    const char* state_str = restage_cl_states_to_str(sptr_clstatus->cl_state());
