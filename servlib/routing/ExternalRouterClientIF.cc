                                                   ...
                                                   RestageCLMap& restage_cl_map)
{
    ...
    status = decode_text_string(entryElement, tmp_str);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_restage_link_name(tmp_str);

    // storage path
    status = decode_text_string(entryElement, tmp_str);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_storage_path(tmp_str);

    // mount point flag
    status = decode_boolean(entryElement, tmp_bool);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_mount_point(tmp_bool);

    //validated mount point 
    status = decode_text_string(entryElement, tmp_str);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_validated_mount_pt(tmp_str);

    // mount point validated flag
    status = decode_boolean(entryElement, tmp_bool);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_mount_pt_validated(tmp_bool);

    // storage path exists flag
    status = decode_boolean(entryElement, tmp_bool);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_storage_path_exists(tmp_bool);

    // part of pool flag
    status = decode_boolean(entryElement, tmp_bool);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_part_of_pool(tmp_bool);

    // Total volume space
    status = decode_uint(entryElement, tmp_u64);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_vol_total_space(tmp_u64);

    // Available volume space
    status = decode_uint(entryElement, tmp_u64);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_vol_space_available(tmp_u64);

    // Max disk quota
    status = decode_uint(entryElement, tmp_u64);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_disk_quota(tmp_u64);

    // Disk quota in use
    status = decode_uint(entryElement, tmp_u64);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_disk_quota_in_use(tmp_u64);

    // Num files restaged
    status = decode_uint(entryElement, tmp_u64);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_disk_num_files(tmp_u64);

    // Num days retention
    status = decode_uint(entryElement, tmp_u64);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_days_retention(tmp_u64);

    // Expire bundles flag
    status = decode_boolean(entryElement, tmp_bool);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_expire_bundles(tmp_bool);

    // Time to live override when reloading bundles
    status = decode_uint(entryElement, tmp_u64);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_ttl_override(tmp_u64);

    // Auto-reload interval (seconds)
    status = decode_uint(entryElement, tmp_u64);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_auto_reload_interval(tmp_u64);

    // current state of the restage CL
    status = decode_text_string(entryElement, tmp_str);
    CHECK_CBORUTIL_STATUS_RETURN
    sptr_clstatus->set_cl_state(str_to_restage_cl_state(tmp_str.c_str()));


    err = cbor_value_leave_container(&mapElement, &entryElement);
    CBORUTIL_CHECK_CBOR_DECODE_ERR_RETURN

    restage_cl_map[sptr_clstatus->restage_link_name()] = sptr_clstatus;
