...
RegistrationInitialLoadThread::RegistrationInitialLoadThread(SPtr_Registration& sptr_reg)
    : Thread("RegistrationInitialLoadThread", DELETE_ON_EXIT )
{
    sptr_reg_ = sptr_reg;
    sptr_reg_->set_initial_load_thread(this);
}
...
RegistrationInitialLoadThread::run()
{
    ...
    //log_always_p("/dtn/reginitload", "run for Reg: *%p", sptgr_reg_.get());
...
RegistrationInitialLoadThread::scan_pending_bundles_map()
{
    ...
    while ( !should_stop() ) {
        ...
        try {
            if (!bref->is_fragment() &&
                sptr_reg_->endpoint()->match(bref->dest())) {
                 ...
                 */
                bref->fwdlog()->add_entry(sptr_reg_.get(),
                                            ForwardingInfo::FORWARD_ACTION,
                                            ForwardingInfo::PENDING_DELIVERY);

                bd->deliver_to_registration(bref.object(), sptr_reg_, true);
                ++count;
            }
