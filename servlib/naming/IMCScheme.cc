...
namespace oasys {
    template <> dtn::IMCScheme* oasys::Singleton<dtn::IMCScheme>::instance_ = 0;
}
...
//----------------------------------------------------------------------
bool
IMCScheme::parse(const std::string& ssp,
...
//----------------------------------------------------------------------
void
IMCScheme::format(std::string* ssp, u_int64_t node, u_int64_t service)
{
...
//----------------------------------------------------------------------
bool
IMCScheme::validate(const URI& uri, bool is_pattern)
{
    if (!uri.valid()) {
        log_debug_p("/dtn/scheme/imc", "IMCScheme::validate: invalid URI");
        return false;
    }
...
//----------------------------------------------------------------------
bool
IMCScheme::match(const EndpointIDPattern& pattern, const EndpointID& eid)
{
    // sanity check
    ...
    if (! IMCScheme::parse(eid_ssp, &eid_node, &eid_service)) {
        return false;
    }
    ...
    } else if (! IMCScheme::parse(pat_ssp, &pat_node, &pat_service)) {
        // the pattern was not a valid numeric pair
        log_debug_p("/dtn/scheme/imc", "IMCScheme::match: invalid pattern: %s",
                    pat_ssp.c_str());
        return false;
    }
...
//----------------------------------------------------------------------
bool
IMCScheme::ipn_node_match(const EndpointID& eid1, const EndpointID& eid2)
{
    ...
    if (! IMCScheme::parse(eid_ssp, &eid1_node, &dummy_service)) {
        return false;
    }

    eid_ssp = eid2.ssp();
    if (! IMCScheme::parse(eid_ssp, &eid2_node, &dummy_service)) {
        return false;
    }
...
//----------------------------------------------------------------------
bool
IMCScheme::append_service_tag(URI* uri, const char* tag)
{
    // the uri must be of the form imc:[node].0, so it replaces the .0
    // with the tag value
    u_int64_t node, service;
    if (! IMCScheme::parse(uri->ssp(), &node, &service)) {
...
//----------------------------------------------------------------------
bool
IMCScheme::append_service_wildcard(URI* uri)
{
    // the uri must be of the form imc:[node].0, so it replaces the .0
    // with the tag value
    u_int64_t node, service;
...
IMCScheme::is_singleton(const URI& uri)
{
    (void)uri;
