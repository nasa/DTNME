...
//----------------------------------------------------------------------
bool
IMCScheme::parse(const SPtr_EID& sptr_eid,
                 size_t* node, size_t* service)
{
    bool result = false;
    if (sptr_eid->valid() && sptr_eid->is_imc_scheme()) {
...
//----------------------------------------------------------------------
void
IMCScheme::format(std::string* ssp, size_t node, size_t service)
{
    oasys::StaticStringBuffer<256> buf("%zu.%zu",
                                       node, service);
...
//----------------------------------------------------------------------
void
IMCScheme::format(SPtr_EID& sptr_eid, size_t group, size_t service)
{
    sptr_eid = BD_MAKE_EID_IMC(group, service);
}
                    ...
                    bool& node_wildcard, bool& service_wildcard)
{
    ...
    if (!uri.valid()) {
        log_debug_p("/dtn/scheme/imc", "IPNScheme::validate: invalid URI");
        return false;
    }
...
//----------------------------------------------------------------------
bool
IMCScheme::match(const EndpointIDPattern& pattern, const SPtr_EID& sptr_eid)
{
    // sanity check
...
//----------------------------------------------------------------------
bool
IMCScheme::ipn_node_match(const SPtr_EID& sptr_eid1, const SPtr_EID& sptr_eid2)
{
...
IMCScheme::eid_dest_type(const URI& uri) 
{
    (void)uri;
