    ...
    if(contactPlan.size() > 0){
      ...
      if(tmp_entry.link_ != NULL){
        ...
        if(diff_seconds >= 0 && diff_seconds <= 5){
          ...
          delete_contact(tmp_entry.sptr_eid_, entry_id);
...
ContactPlanner::get_plan(SPtr_EID& sptr_eid)
{
  ...
  {
    if (sptr_eid == it->sptr_eid_)
    {
      int tmp_id;
      tmp_id = it->id_;
      struct Contact_Entry tmp_entry;
      tmp_entry.id_ = tmp_id;
      tmp_entry.sptr_eid_ = it->sptr_eid_; 
...
//----------------------------------------------------------------------
std::vector<dtn::ContactPlanner::Contact_Entry>
ContactPlanner::get_contact(SPtr_EID& sptr_eid, std::string id_range)
{
  int start, end;
  char colon;
  std::stringstream ss;
...
//----------------------------------------------------------------------
bool
ContactPlanner::export_plan(SPtr_EID& sptr_eid, std::string filename)
{
    ...
    while(std::getline(mapfile,line)){
      ...
      while(std::getline(lineStream,token,',')){
       if(!token.empty()){
         switch(count){
           case 0:
             row.sptr_eid_ = BD_MAKE_EID(token);
             break;
           case 1:
...
//----------------------------------------------------------------------
void
ContactPlanner::update_plan_orig()
{
  int id = 1;
  contactPlan.clear();
  ...
  for (std::vector<Contact_Entry>::iterator it = contactList.begin(); it != contactList.end(); ++it)
  {
    it->id_ = id;
    id++;
    if (it->sptr_eid_ == BundleDaemon::instance()->local_eid_ipn())
    {
      struct Contact_Entry tmp_entry;
...
{
  SPtr_EID sptr_local_eid = BundleDaemon::instance()->local_eid_ipn();
  SPtr_EID sptr_test_eid;
...
//----------------------------------------------------------------------
void
ContactPlanner::add_contact(SPtr_EID& sptr_eid, std::string link_name, std::string start_time, std::string duration)
{
  struct Contact_Entry row;

  row.sptr_eid_ = sptr_eid;
...
//----------------------------------------------------------------------
void
ContactPlanner::delete_contact(SPtr_EID& sptr_eid, std::string id_range)
{
  int start, end;
  char colon;
  std::stringstream ss;
