...
Link::cancel_all_bundles()
{
    ...
    while (bref != nullptr) {
        ...
        event_to_post = new BundleSendCancelledEvent(bref.object(), lref);
    ...
    while (bref != nullptr) {
        ...
        // sanity checks
        // It can happen:  ASSERT(payload_len != 0);
...
Link::set_used_in_fwdlog()
{
    ...
    if (!used_in_fwdlog_)
    {
        ...
        event_to_post = new StoreLinkUpdateEvent(this);
        SPtr_BundleEvent sptr_event_to_post(event_to_post);
        BundleDaemon::post(sptr_event_to_post);
    }
...
Link::parse_args(int argc, const char* argv[], const char** invalidp)
{
    ...
    p.addopt(new dtn::EndpointIDOpt("remote_eid", &sptr_remote_eid_));
...
Link::add_to_queue(const BundleRef& bundle)
{
    ...
    if ((state_ == UNAVAILABLE) && is_opportunistic()) {
        if (BundleDaemon::params_.clear_bundles_when_opp_link_unavailable_) {
            ...
            BundleSendCancelledEvent* event_to_post;
            event_to_post = new BundleSendCancelledEvent(bundle.object(), lref);
            SPtr_BundleEvent sptr_event_to_post(event_to_post);
            BundleDaemon::post(sptr_event_to_post);
            return false;
        }
    ...
    {
        oasys::ScopeLock scoplock(contact_->lock(), __func__);
...
Link::format(char* buf, size_t sz) const
{
    return snprintf(buf, sz, "%s [%s %s %s %s state=%s]",
                    name(), nexthop(), sptr_remote_eid_->c_str(),
...
Link::dump(oasys::StringBuffer* buf)
{
    ...
    if( !params_.qlimit_enabled_ ) {
        buf->append("queue limits disabled\n");
    } else {
...
Link::dump_stats(oasys::StringBuffer* buf)
{
    ...
    if(planner_contact_!=NULL){	
      ...
      buf->append("\n\nContact Info");
      buf->append("\nStart Time: ");
      buf->append(start_time.c_str());
      buf->append(" - Duration: ");
      buf->append(duration.c_str());
