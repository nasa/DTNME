...
BP7_PreviousNodeBlockProcessor::decode_block_data(uint8_t* data_buf, size_t data_len, Bundle* bundle)
{
    ...
    int64_t status = decode_eid(cvElement, bundle->mutable_prevhop(), fld_name);
                                                  ...
                                                  const Bundle* bundle, int64_t& encoded_len)
{
    ...
    std::string fld_name = "BP7_PreviousNodeBlockProcessor::encode_block_data";
        ...
        //log_always_p(log_path(), "dzdebug - %s - Encoding Previous Node: %s", __func__, eidref.c_str());

    int64_t status = encode_eid(encoder, sptr_eid, bundle, fld_name);
    CHECK_BP7_STATUS_RETURN
                                ...
                                bool           last)
{
    ...
    SPtr_EID sptr_announce_eid;
    if (bd->params_.announce_ipn_) {
        sptr_announce_eid = bd->local_eid_ipn();
    } else {
        sptr_announce_eid = bd->local_eid();
    }
    int64_t status;
    int64_t data_len;
    int64_t encoded_len;
    ...
    data_len = encode_block_data(nullptr, 0, sptr_announce_eid, bundle, encoded_len);
    if (BP_FAIL == data_len)
    {
    ...
    status = encode_block_data(data_buf, data_len, sptr_announce_eid, bundle, encoded_len);
