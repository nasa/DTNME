...
int64_t
BP7_PayloadBlockProcessor::decode_payload_header(SPtr_BlockInfo& block, 
                               ...
                               size_t     len)
{
    ...
    if (buf[0] != 0xff)
    {
        log_err_p(log_path(), "BP7 error: CBOR break character (0xff) expected after Payload Blocki - got: %2.2x", buf[0]);
        return BP_FAIL;
    }
