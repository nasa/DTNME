...
#ifndef _BUNDLE_ARCHITECTURAL_RESTAGING_DAEMON_H_
#define _BUNDLE_ARCHITECTURAL_RESTAGING_DAEMON_H_
...
//dzdebug class BundleArchitecturalRestagingDaemon;
...
public:
   ...
   /**
    ...
    struct Params {
        ...
        /// whether or not to send email notifications when watermark thresholds are triggered
        bool email_notifications_enabled_ = true;

        /// Limit emails of each type to 1 per frequency time period starting when first message is sent
        size_t email_frequency_ = 600;

        /// Internal storage Low watermark percentage for issuing email notifications
        size_t internal_low_watermark_ = 25;

        /// Internal storage high watermark percentage for issuing email notifications
        size_t internal_high_watermark_ = 75;

        /// External storage Low watermark percentage for issuing email notifications
        size_t external_low_watermark_ = 25;

        /// External storage high watermark percentage for issuing email notifications
        size_t external_high_watermark_ = 75;

        /// Whether or not BARD is enabled
        bool enabled_ = true;

        /// Whether to treat unknown link as pooled
        bool treat_unknown_link_as_pooled_ = true;
    };
    ...
    bool query_accept_bundle_internal_storage_only(Bundle* bundle);
    ...
    void bardcmd_committed(oasys::StringBuffer& buf);
    void bardcmd_committed_exact(oasys::StringBuffer& buf);
    ...
    bool bardcmd_get_internal_low_watermark(std::string& result_str);
    bool bardcmd_set_internal_low_watermark(size_t new_watermark, std::string& result_str);

    bool bardcmd_get_internal_high_watermark(std::string& result_str);
    bool bardcmd_set_internal_high_watermark(size_t new_watermark, std::string& result_str);

    bool bardcmd_get_external_low_watermark(std::string& result_str);
    bool bardcmd_set_external_low_watermark(size_t new_watermark, std::string& result_str);

    bool bardcmd_get_external_high_watermark(std::string& result_str);
    bool bardcmd_set_external_high_watermark(size_t new_watermark, std::string& result_str);
...
protected:
     ...
     */
    bool query_accept_bundle_by_destination_into_internal_storage(Bundle* bundle, bool reserve_space);
    bool query_accept_bundle_by_source_into_internal_storage(Bundle* bundle, bool reserve_space);
    bool query_accept_bundle_into_internal_storage(std::string& key, bool restage_by_src, Bundle* bundle, bool reserve_space);

    bool query_accept_bundle_by_destination_into_external_storage(Bundle* bundle);
    bool query_accept_bundle_by_source_into_external_storage(Bundle* bundle);
    bool query_accept_bundle_into_external_storage(std::string& key, bool restage_by_src, Bundle* bundle);
    ...
    bool find_restage_link_with_email(std::string& link_name);
    const char* get_restage_link_state_str(std::string link_name);

    void generate_email_notification(SPtr_BARDNodeStorageUsage sptr_usage,
                                     const char* storage_msg,
                                     const char* watermark_msg,
                                     size_t watermark_val,
                                     bool& email_sent_flag);

    void send_restage_link_email(std::string link_name, std::string& message,
                                 std::string& message2);
    ...
    std::string format_internal_quota_committed(BARDNodeStorageUsage* usage_ptr);
    std::string format_external_quota_committed(BARDNodeStorageUsage* usage_ptr);

    size_t max_inuse_quota_percent(BARDNodeStorageUsage* usage_ptr);
    size_t max_internal_committed_quota_percent(BARDNodeStorageUsage* usage_ptr);
    size_t max_external_committed_quota_percent(BARDNodeStorageUsage* usage_ptr);

    /**
     * Utility function to generate a BARDNodeStorageUsage key string given an Endpoint ID
     ...
     * @param sptr_eid The Endpoint ID associrated with the node; The scheme and name/node number are extracted from the EID
     */
    std::string make_bardnodestorageusage_key(bard_quota_type_t quota_type, const SPtr_EID& sptr_eid);

    /**
     * Utility function to translate an endpoint into the BARDNodeStorageUsage scheme and nodename components
     * (not part of the BARDNodeStorageUsage to minimize the external router client side dependencies)
     *
     * @param sptr_eid The Endpoint ID to parse
     * @param scheme The scheme type of the Endpoint ID
     * @param nodename The nodename of the Endpoint ID
     */
    void endpoint_to_key_components(const SPtr_EID& sptr_eid, bard_quota_naming_schemes_t& scheme, 
                                    std::string& nodename);
...
protected:
    ...
    /// Flags used to keep BARD from sending emails except for once every threshold crossing 
    bool bard_external_threshold_low_reached_ = false;
    bool bard_external_threshold_high_reached_ = false;
    ...
    std::string last_link_name_used_for_email_; ///< Last Restage CL Link used to send an email notification
    ...
    size_t internal_storage_block_size_ = 4096; ///< Block size used by the internal storage file system
};
...
#endif /* _BUNDLE_ARCHITECTURAL_RESTAGING_DAEMON_H_ */
