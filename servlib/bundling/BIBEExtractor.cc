...
//----------------------------------------------------------------------
void
BIBEExtractor::post(Bundle* bundle, SPtr_Registration& sptr_reg)
{
...
BIBEExtractor::process_event(BibeEvent* event)
{
    if (extract_bundle(event->bref_.object())) {
        event->bref_->fwdlog()->update( event->sptr_reg_.get(), ForwardingInfo::DELIVERED);
    ...
    event_to_post = new BundleDeliveredEvent(event->bref_.object(), event->sptr_reg_);
...
BIBEExtractor::extract_bundle(Bundle* bibe_bundle)
{
    ...
    bptr->mutable_prevhop() = bibe_bundle->prevhop();


    BundleReceivedEvent* event_to_post;
    event_to_post = new BundleReceivedEvent(bptr.release(), EVENTSRC_PEER, 
                                            payload_len, sptr_remote_eid, link.object());
    SPtr_BundleEvent sptr_event_to_post(event_to_post);
    BundleDaemon::post(sptr_event_to_post);
