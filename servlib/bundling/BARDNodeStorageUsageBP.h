...
#ifndef _BARD_NODE_STORAGE_USAGE_BP_H_
#define _BARD_NODE_STORAGE_USAGE_BP_H_
...
#include "BARDNodeStorageUsageBP.h"
...
/**
 * The BARDNodeStorageUsageBP class contains both quota and usage tracking
 * data for a specifc source or destination node. Only the quota variables
 * are serialized for storage in a database table.
 */
class BARDNodeStorageUsageBP: public BARDNodeStorageUsage
{
public:
    ...
    /**
     * Static function to generate a key string given an EndpointID
     *
     * @param quota_type Indicates whether the key if for a Destination or Source Endpoint ID
     * @param eid The EndpointID associrated with the node; The scheme and name/node number are extracted from the eid
     */
    static std::string make_key(bard_quota_type_t quota_type, const EndpointID& eid);


    /**
     * Constructor
     */
    BARDNodeStorageUsageBP();

    /**
     * Constructor
     */
    BARDNodeStorageUsageBP(bard_quota_type_t quota_type, const EndpointID& eid);

    /**
     * Constructor
     */
    BARDNodeStorageUsageBP(bard_quota_type_t quota_type, 
     ...
     */
    BARDNodeStorageUsageBP(const oasys::Builder&);
     ...
     */
    bool operator< (const BARDNodeStorageUsageBP& other);
    ...
    void set_endpoint(const EndpointID& eid);
...
typedef std::shared_ptr<BARDNodeStorageUsageBP>             SPtr_BARDNodeStorageUsageBP;

typedef std::map<std::string, SPtr_BARDNodeStorageUsageBP>  BARDNodeStorageUsageBPMap;

} // namespace dtn

#endif   // BARD_ENABLED

#endif /* _BARD_NODE_STORAGE_USAGE_BP_H_ */
