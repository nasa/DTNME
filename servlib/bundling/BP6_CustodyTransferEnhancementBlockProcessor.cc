...
//
BP6_CustodyTransferEnhancementBlockProcessor::BP6_CustodyTransferEnhancementBlockProcessor()
    : BP6_BlockProcessor(BundleProtocolVersion6::CUSTODY_TRANSFER_ENHANCEMENT_BLOCK) 
{
	block_name_ = "BP6_CustodyTransferEnhancement";
}
...
// BSP?)
int
BP6_CustodyTransferEnhancementBlockProcessor::prepare(const Bundle*    bundle,
...
// blocks?
int
BP6_CustodyTransferEnhancementBlockProcessor::generate(const Bundle*  bundle,
                            BlockInfoVec*  xmit_blocks,
                            BlockInfo*     block,
                            const LinkRef& link,
                            bool           last)
...
ssize_t
BP6_CustodyTransferEnhancementBlockProcessor::consume(Bundle*    bundle,
                           BlockInfo* block,
                           u_char*    buf,
                           size_t     len)
{
    ...
    if ( ! is_cteb_creator_custodian((const char*)bundle->custodian()->c_str(), cteb_creator) ) {
       ...
       log_debug_p(LOG, "Invalid CTEB recevied - removing block: "
       ...
       log_debug_p(LOG, "Received bundle from ACS capable server: %s  "
...
bool
BP6_CustodyTransferEnhancementBlockProcessor::is_cteb_creator_custodian(const char* custodian, const char* creator) {
    bool result = false;
