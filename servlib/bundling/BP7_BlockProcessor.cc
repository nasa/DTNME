...
//----------------------------------------------------------------------
ssize_t
BP7_BlockProcessor::consume_preamble(SPtr_BlockInfoVec& recv_blocks,
...
//----------------------------------------------------------------------
void
BP7_BlockProcessor::generate_preamble(SPtr_BlockInfoVec& xmit_blocks,
                                  SPtr_BlockInfo&    block,
                                  u_int8_t      type,
                                  u_int64_t     flags,
...
//----------------------------------------------------------------------
void
BP7_BlockProcessor::init_block(SPtr_BlockInfo&    block,
                           SPtr_BlockInfoVec& block_list,
                           Bundle*       bundle,
                           u_int8_t      type,
                           u_int8_t      flags,
...
//----------------------------------------------------------------------
int
BP7_BlockProcessor::format(oasys::StringBuffer* buf, BlockInfo *b)
{
	(void) b;

	buf->append("BP7 Generic Block Processor (Base Class)");
	return 0;
}
...
//
int64_t
BP7_BlockProcessor::encode_canonical_block(uint8_t* buf, uint64_t buflen, 
                                                ...
                                                uint8_t* data_buf, int64_t data_len, int64_t& encoded_len)
{
    ...
    // CRC type
    uint64_t crc_type = block->crc_type();
...
//----------------------------------------------------------------------
int
BP7_BlockProcessor::encode_eid_dtn(CborEncoder& blockArrayEncoder, const EndpointID& eidref, const Bundle* bundle, std::string& fld_name)
{
    CborError err;
    CborEncoder eidArrayEncoder;
...
//----------------------------------------------------------------------
int
BP7_BlockProcessor::encode_eid_ipn(CborEncoder& blockArrayEncoder, const EndpointID& eidref, const Bundle* bundle, std::string& fld_name)
{
    CborError err;
    ...
    {
        log_crit_p(log_path(), "Error parsing IPN scheme to node and service components: %s", eidref.c_str());
        return BP_FAIL;
    }
...
//----------------------------------------------------------------------
int
BP7_BlockProcessor::encode_eid(CborEncoder& blockArrayEncoder, const EndpointID& eidref, const Bundle* bundle, std::string& fld_name)
{
    ...
    {
        return encode_eid_ipn(blockArrayEncoder, eidref, bundle, fld_name);
    }
    else if (eidref.scheme() == DTNScheme::instance())
    {
        return encode_eid_dtn(blockArrayEncoder, eidref, bundle, fld_name);
    }
    else
    {
        log_crit_p(log_path(), "Unrecognized EndpointID Scheme: %s", eidref.c_str());
        return BP_FAIL;
    }
}
...
//----------------------------------------------------------------------
bool
BP7_BlockProcessor::data_available_to_parse(CborValue& cvElement, uint64_t len)
{
...
//     BP7_UNEXPECTED_EOF (1) = need more data to parse
int
BP7_BlockProcessor::decode_length_of_fixed_string(CborValue& cvElement, uint64_t& ret_val, 
                                                  std::string& fld_name, uint64_t& decoded_bytes)
{
    if (!data_available_to_parse(cvElement, 1))
    {
        return BP7_UNEXPECTED_EOF;
...
//     BP7_UNEXPECTED_EOF (1) = need more data to parse
int
BP7_BlockProcessor::decode_uint(CborValue& cvElement, uint64_t& ret_val, std::string& fld_name)
{
    if (!data_available_to_parse(cvElement, 1))
    {
        return BP7_UNEXPECTED_EOF;
    ...
    {
        log_err_p(log_path(), "BP7 decode Error: %s - must be encoded as an array", fld_name.c_str());
        return BP_FAIL;
    }
                                      ...
                                      uint64_t min_len, uint64_t max_len, uint64_t& num_elements, std::string& fld_name)
{
    ...
    {
        log_err_p(log_path(), "BP7 decode Error: %s - must be encoded as an array - CBOR byte = %2.2x", 
                              fld_name.c_str(), *cvElement.ptr);
        return BP_FAIL;
    }
...
//     BP7_UNEXPECTED_EOF (1) = need more data to parse
int
BP7_BlockProcessor::decode_eid(CborValue& cvElement, EndpointID& eid, std::string& fld_name)
{
    ...
    if (schema_type == 1)
    {
        ...
        {
            status = decode_uint(cvEidElements, node_id, fld_name);
            CHECK_BP7_STATUS_RETURN
    ...
    else if (2 == schema_type)
    {
        ...
        status = decode_uint(cvSspElements, node_id, fld_name);
        CHECK_BP7_STATUS_RETURN
...
//     BP7_UNEXPECTED_EOF (1) = need more data to parse
int
BP7_BlockProcessor::decode_crc_and_validate(CborValue& cvElement, uint64_t crc_type, 
...
//----------------------------------------------------------------------
int64_t
BP7_BlockProcessor::uint64_encoding_len(uint64_t val)
{
