...
static const char* log = "/dtn/bundle/bp6";

//----------------------------------------------------------------------
BP6_BlockProcessor::BP6_BlockProcessor(int block_type)
    : BlockProcessor(block_type)
{
    log_path_ = "/dtn/bp6";
	block_name_ = "BP6 Generic Block Processor (Base Class)";

    set_bp_version(BundleProtocol::BP_VERSION_6);
}

//----------------------------------------------------------------------
BP6_BlockProcessor::~BP6_BlockProcessor()
{
}

//----------------------------------------------------------------------
ssize_t
BP6_BlockProcessor::consume_preamble(BlockInfoVec* recv_blocks,
                                 BlockInfo*    block,
                                 u_char*       buf,
                                 size_t        len,
                                 u_int64_t*    flagp)
{
    ...
    // parse it.
    if (contents->len() <= BundleProtocolVersion6::PREAMBLE_FIXED_LENGTH) {
//dz debug        ASSERT(tocopy == len);
    ...
    size_t buf_offset = BundleProtocolVersion6::PREAMBLE_FIXED_LENGTH;
    u_int64_t flags;
    ...
    if ( flags & BundleProtocolVersion6::BLOCK_FLAG_EID_REFS ) {
        sdnv_len = SDNV::decode(contents->buf() + buf_offset,
                                contents->len() - buf_offset,
                                &eid_ref_count);
        if (sdnv_len == -1) {
        ...
        for ( u_int32_t i = 0; i < eid_ref_count; ++i ) {
            ...
            SPtr_EID sptr_eid;
            dict->extract_eid(sptr_eid, scheme_offset, ssp_offset);
            eid_list.push_back(sptr_eid);
        }
    }
    ...
    log_debug_p(log, "BP6_BlockProcessor type 0x%x "
...
//----------------------------------------------------------------------
void
BP6_BlockProcessor::generate_preamble(BlockInfoVec* xmit_blocks,
                                  BlockInfo*    block,
                                  u_int8_t      type,
                                  u_int64_t     flags,
                                  u_int64_t     data_length)
{
    ...
    if ( eid_count > 0 ) {
        flags |= BundleProtocolVersion6::BLOCK_FLAG_EID_REFS;
    ...
    ASSERT(block->contents().buf_len() >= BundleProtocolVersion6::PREAMBLE_FIXED_LENGTH 
    ...
    *bp = type;
    bp  += BundleProtocolVersion6::PREAMBLE_FIXED_LENGTH;
    len -= BundleProtocolVersion6::PREAMBLE_FIXED_LENGTH;
    ...
    u_int32_t offset = BundleProtocolVersion6::PREAMBLE_FIXED_LENGTH + 
                       flag_sdnv_len + eid_field_len + length_sdnv_len;
    ...
    log_debug_p(log, "BP6_BlockProcessor type 0x%x "
                ...
                block_type(), block->type(), U64FMT(block->block_flags()),
                block->data_offset(), block->data_length(), eid_count);
}

//----------------------------------------------------------------------
int64_t
BP6_BlockProcessor::consume(Bundle*    bundle,
                        BlockInfo* block,
                        u_char*    buf,
                        size_t     len)
{
    ...
    if (block->data_offset() == 0) {
        int cc = consume_preamble(recv_blocks.get(), block, buf, len);
        if (cc == -1) {
            return -1;
        }
...
//----------------------------------------------------------------------
bool
BP6_BlockProcessor::validate(const Bundle*           bundle,
                         ...
                         status_report_reason_t* deletion_reason)
{
    ...
    if (bundle->is_admin() &&
        block->type() != BundleProtocolVersion6::PRIMARY_BLOCK &&
        block->block_flags() & BundleProtocolVersion6::BLOCK_FLAG_REPORT_ONERROR) {
        log_err_p(log, "invalid block flag 0x%x for received admin bundle",
                  BundleProtocolVersion6::BLOCK_FLAG_REPORT_ONERROR);
...
//----------------------------------------------------------------------
int
BP6_BlockProcessor::reload_post_process(Bundle*       bundle,
                                    BlockInfoVec* block_list,
                                    BlockInfo*    block)
{
    (void)bundle;
...
//----------------------------------------------------------------------
int
BP6_BlockProcessor::prepare(const Bundle*    bundle,
                        ...
                        list_owner_t     list)
{
    ...
    if (list == BlockInfo::LIST_RECEIVED) {
        ...
        ASSERT(blkptr->type() == BundleProtocolVersion6::PRIMARY_BLOCK);
        xmit_blocks->insert(iter + 1, std::make_shared<BlockInfo>(this, source));
    }
    return BP_SUCCESS;
}
...
//----------------------------------------------------------------------
int
BP6_BlockProcessor::finalize(const Bundle*  bundle,
                         BlockInfoVec*  xmit_blocks,
                         BlockInfo*     block,
                         const LinkRef& link)
{
    ...
    if (bundle->is_admin() && block->type() != BundleProtocolVersion6::PRIMARY_BLOCK) {
        ASSERT((block->block_flags() &
                BundleProtocolVersion6::BLOCK_FLAG_REPORT_ONERROR) == 0);
    }
    return BP_SUCCESS;
}
...
//----------------------------------------------------------------------
void
BP6_BlockProcessor::process(process_func*    func,
                        const Bundle*    bundle,
...
//----------------------------------------------------------------------
bool
BP6_BlockProcessor::mutate(mutate_func*     func,
                       Bundle*          bundle,
                       const BlockInfo* caller_block,
...
//----------------------------------------------------------------------
void
BP6_BlockProcessor::produce(const Bundle*    bundle,
...
//----------------------------------------------------------------------
void
BP6_BlockProcessor::init_block(BlockInfo*    block,
                           BlockInfoVec* block_list,
                           Bundle*       bundle,
                           u_int8_t      type,
                           u_int8_t      flags,
