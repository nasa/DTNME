...
int64_t
BP7_PrimaryBlockProcessor::decode_cbor(Bundle* bundle, BlockInfo* block, uint8_t*  buf, size_t buflen)
{
    CborError err;
    CborParser parser;
    ...
    status = decode_eid(cvBlockElements, bundle->mutable_dest(), fld_name);
    CHECK_BP7_STATUS_RETURN

    SET_FLDNAMES("Source EID");
    SPtr_EID sptr_eid;
    status = decode_eid(cvBlockElements, sptr_eid, fld_name);
    CHECK_BP7_STATUS_RETURN
    bundle->set_source(sptr_eid);

    SET_FLDNAMES("Report-To EID");
    status = decode_eid(cvBlockElements, bundle->mutable_replyto(), fld_name);
    CHECK_BP7_STATUS_RETURN
