...
{
    SPtr_BlockInfoVec api_blocks = bundle->api_blocks();
         ...
         ++iter)
    {
        ...
        blkptr->owner()->reload_post_process(bundle, api_blocks, blkptr);
    }
         ...
         ++iter)
    {
        ...
        // things needed after a load-from-store
        //log_debug_p(LOG, "reload_post_process on bundle of type %d", blkptr->owner()->block_type());
        blkptr->owner()->reload_post_process(bundle, recv_blocks, blkptr);
    }
}
...
BundleProtocolVersion7::prepare_blocks(Bundle* bundle, const LinkRef& link)
{
    ...
    // arrived on the link
    SPtr_BlockInfoVec xmit_blocks = bundle->xmit_blocks()->create_blocks(link);
    SPtr_BlockInfoVec recv_blocks = bundle->recv_blocks();
    SPtr_BlockInfoVec api_blocks  = bundle->api_blocks();
    BlockProcessor* bp;
    int i;
    ...
    if (recv_blocks->size() > 0) {
        ...
        if (recv_blocks->front()->type() != PRIMARY_BLOCK) {
            //dzdebug
            log_crit_p("/bp7", "prepare_blocks - bundle %" PRIbid " had recv_blocks:  %zu", bundle->bundleid(), recv_blocks->size());

            log_crit_p("/bp7", "prepare_blocks - first block of %zu is not a primary block - got 0x%2.2x", 
                       recv_blocks->size(), recv_blocks->front()->type());

            int ctr = 0;
            for (recv_iter = recv_blocks->begin();
                 recv_iter != recv_blocks->end();
                 ++recv_iter)
            {
                SPtr_BlockInfo blkptr = *recv_iter;
                log_crit_p("/bp7", "prepare_blocks - Block #%d is type 0x%2.2x", ctr, recv_blocks->front()->type());
                ctr++;
            }
             ...
             ++recv_iter)
        {
            ...
            if (bundle->fragmented_incoming()
                && xmit_blocks->find_block(BundleProtocolVersion7::PAYLOAD_BLOCK)) {
                continue;
            }
            
            if (BP_FAIL == blkptr->owner()->prepare(bundle, xmit_blocks, blkptr, link,
    ...
    for (api_iter = api_blocks->begin();
         api_iter != api_blocks->end();
         ++api_iter)
    {
        SPtr_BlockInfo blkptr = *api_iter;

        //log_debug_p(LOG, "adding api_block");
        if(BP_FAIL == blkptr->owner()->prepare(bundle, xmit_blocks, 
                                ...
                                const LinkRef& link)
{
         ...
         ++iter)
    {
        ...
        if(BP_FAIL == blkptr->owner()->generate(bundle, blocks, blkptr, link, last)) {
         ...
         ++iter)
    {
        ...
        if(BP_FAIL == blkptr->owner()->finalize(bundle, blocks, blkptr, link)) {
...
//----------------------------------------------------------------------
size_t
BundleProtocolVersion7::total_length(Bundle* bundle, const SPtr_BlockInfoVec& blocks)
{
    (void) bundle;
...
//----------------------------------------------------------------------
size_t
BundleProtocolVersion7::payload_offset(const SPtr_BlockInfoVec& blocks)
{
    size_t ret = 0;
    for (BlockInfoVec::const_iterator iter = blocks->begin();
         iter != blocks->end();
...
//----------------------------------------------------------------------
size_t
BundleProtocolVersion7::produce(const Bundle* bundle, const SPtr_BlockInfoVec& blocks,
                        u_char* data, size_t offset, size_t len, bool* last)
{
    ...
    *last = false;
    ...
    if (iter == blocks->end())
    {
        ...
        data[0] = 0x1f;  // 31 is the CBOR break value
        *last = true;
        return 1;
    }
    ...
    while (1) {
        ...
        blkptr->owner()->produce(bundle, blkptr, data, offset, tocopy);
    ...
    if (len > 0) {
        ...
        data[0] = 0xff;  // 0xff is the CBOR break value
        len -= 1;
        *last = true;
    }
                        ...
                        bool*   last)
{
    ...
    *last = false;
    ...
    while (len != 0) {
        ...
        SPtr_BlockInfo info = recv_blocks->back();
        ...
        if (info->complete()) {
            ...
            //log_always_p(LOG, "BundleProtocolVersion7::consume - peeking at block type - bytes to process: %zu",
            //             len);
            ...
            if (BP_SUCCESS != status)
            {
                if (BP7_UNEXPECTED_EOF == status) {
                    //log_err_p(LOG, "BundleProtocolVersion7::consume - error peek at next block type - need more data (origlen= %zu len= %zu) first byte: %2.2x",
                    //          origlen, len, data[0]);
                    return origlen - len;
                } else {
            ...
            info = recv_blocks->append_block(find_processor(block_type));
        ...
        ssize_t cc = info->owner()->consume(bundle, info, data, len);
        if (cc < 0) {
                         ...
                         BundleProtocol::status_report_reason_t* deletion_reason)
{
    ...
    if (recv_blocks->front()->type() != BundleProtocolVersion7::PRIMARY_BLOCK) {
         ...
         ++iter)
    {
        ...
        if (!blkptr->owner()->validate(bundle, recv_blocks, blkptr, 
...
//----------------------------------------------------------------------
bool
BundleProtocolVersion7::get_admin_type(const Bundle* bundle, admin_record_type_t* type)
{
    if (! bundle->is_admin()) {
        return false;
    }
