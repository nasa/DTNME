...
public:
    GbofId();
    GbofId(const SPtr_EID& sptr_src,
     ...
     */
    bool equals(const SPtr_EID& sptr_src,
                BundleTimestamp creation_ts,
                bool is_fragment,
                size_t frag_len,
    ...
    /// @{ Accessors
    const SPtr_EID source()              const { return sptr_source_; }
    ...
    void set_source(const SPtr_EID& sptr_eid);
    void set_source(const std::string& eid);
    void set_creation_ts(const BundleTimestamp& ts);
    void set_creation_ts(size_t secs_or_millisecs, size_t seqno);
...
private:
    ...
    SPtr_EID sptr_source_;        ///< Source eid
