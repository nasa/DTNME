...
BundleDaemon::get_bundle_stats(oasys::StringBuffer* buf)
{
    ...
    buf->appendf("Totals - BPv7 bundles: %" PRIu64
                 "  BPv6 bundles: %" PRIu64
                 "\n\n",
    ...
    buf->appendf("BIBE in BP6 - encapsulations: %" PRIu64 " extractions: %" PRIu64 " extraction errors: %" PRIu64 "\n",
                 bibe6_encapsulations_, bibe6_extractions_, bibe6_extraction_errors_);
                 
    buf->appendf("BIBE in BP7 - encapsulations: %" PRIu64 " extractions: %" PRIu64 " extraction errors: %" PRIu64 "\n\n",
...
BundleDaemon::get_ltp_object_stats(oasys::StringBuffer* buf)
{
    ...
    buf->appendf("    LtpSessions created: %zu deleted: %zu\n",
                 ltpsession_created_, ltpsession_deleted_);
    buf->appendf("    LtpSegments created: %zu deleted: %zu\n",
                 ltpseg_created_, ltpseg_deleted_);

    buf->appendf("    LtpTimers - Retransmit - created: %zu deleted: %zu\n", 
                 ltp_retran_timers_created_, ltp_retran_timers_deleted_);
    buf->appendf("    LtpTimers - Inactivity - created: %zu deleted: %zu\n", 
                 ltp_inactivity_timers_created_, ltp_inactivity_timers_deleted_);
    buf->appendf("    LtpTimers - Closeout   - created: %zu deleted: %zu\n\n", 
                 ltp_closeout_timers_created_, ltp_closeout_timers_deleted_);
}
...
BundleDaemon::handle_shutdown_request(ShutdownRequest* request)
{
    ...
    do {
        ...
        oasys::ScopeLock scoplok(contactmgr_->lock(), __func__);
        ...
        while (iter != links->end())
        {
            ...
            if (link->isopen()) {
                ...
                //dzdebug
                log_always("BD::handle_shutdown_req - %s - not open?? not calling link->close()", link->name());
            }
    ...
    while (!daemon_output_->is_stopped()) {
        usleep(100000);
    }
    ...
    while (!daemon_input_->is_stopped()) {
        usleep(100000);
    }
    ...
    daemon_cleanup_->set_should_stop();
    while (!daemon_cleanup_->is_stopped()) {
        usleep(100000);
    }
...
BundleDaemon::try_to_delete(const BundleRef& bundle, const char* event_type)
{
    ...
    if (! bundle->is_queued_on(pending_bundles_))
    {
        if (bundle->expired()) {
#ifdef BD_LOG_DEBUG_ENABLED
            log_debug("try_to_delete(%s - *%p): bundle already expired",
...
#ifdef BD_LOG_DEBUG_ENABLED
        // this happens if bundle delivered locally as this is called in the
        // post processing for DELIVERED event and then RECEIVED event 
        log_debug("try_to_delete(%s - *%p): bundle not in pending list!",
                  event_type, bundle.object());
#endif
        return false;
    }
