...
BARDForceRestageThread::scan_pending_bundles_map()
{
    ...
    while (!bd->shutting_down() && (bref != nullptr) && 
        ...
        if (is_bundle_to_restage(bref)) {
            ...
            BundleSendRequest *request = new BundleSendRequest(bref, restage_link_name_, action, true);
...
bool
BARDForceRestageThread::is_bundle_to_restage(const EndpointID& eid)
{
    bool do_restage = false;

    switch (scheme_) {
        case BARD_QUOTA_NAMING_SCHEME_IPN:
            do_restage = (eid.scheme() == IPNScheme::instance());
            if (do_restage) {
                uint64_t node_num = 0;
                uint64_t service_num = 0;
                if (IPNScheme::parse(eid, &node_num, &service_num)) {
                    do_restage = (node_num == node_number_);
                }else {
                    do_restage = false;
                }
            }
        ...
        case BARD_QUOTA_NAMING_SCHEME_DTN:
            do_restage = (eid.scheme() == DTNScheme::instance());
            if (do_restage) {
                do_restage = (nodename_.compare(eid.ssp()) == 0);
            }
            break;

        case BARD_QUOTA_NAMING_SCHEME_IMC:
            do_restage = (eid.scheme() == IMCScheme::instance());
            if (do_restage) {
                uint64_t node_num = 0;
                uint64_t service_num = 0;
