...
BP7_BundleAgeBlockProcessor::decode_block_data(uint8_t* data_buf, size_t data_len, Bundle* bundle)
{
    ...
    bundle->set_prev_bundle_age_millis(age_millis);
        ...
        //log_always_p(log_path(), "BP7 Bundle Age Block - previous age = %" PRIu64, age_millis);
...
//     BP7_UNEXPECTED_EOF (1) = need more data to parse
int64_t
BP7_BundleAgeBlockProcessor::decode_cbor(SPtr_BlockInfo& block, uint8_t*  buf, size_t buflen)
{
    CborError err;
    CborParser parser;
    ...
    std::string fld_name = "BP7_BundleAgeBlockProcessor::decode_cbor";
...
BP7_BundleAgeBlockProcessor::prepare(const Bundle*    bundle,
                               ...
                               list_owner_t     list)
{
    //dzdebug if (list == BlockInfo::LIST_RECEIVED || bundle-> ) {
...
//
int64_t
BP7_BundleAgeBlockProcessor::encode_cbor(uint8_t* buf, uint64_t buflen, const SPtr_BlockInfo& block, const Bundle* bundle,
                                            uint8_t* data_buf, int64_t data_len, int64_t& encoded_len)
{
    ...
    std::string fld_name = "BP7_BundleAgeBlockProcessor::encode_cbor";
    ...
    if (block_first_cbor_byte != nullptr) {
        ...
        {
            log_crit_p(log_path(), "Error - First byte of Bundle Age Block CBOR does not equal expected value (%2.2x): %2.2x", 
...
//----------------------------------------------------------------------
int
BP7_BundleAgeBlockProcessor::format(oasys::StringBuffer* buf, BlockInfo *b)
{
    (void) b;
    return buf->append("BundleAge");
}
