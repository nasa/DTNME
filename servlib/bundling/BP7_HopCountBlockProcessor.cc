...
//----------------------------------------------------------------------
BP7_HopCountBlockProcessor::BP7_HopCountBlockProcessor()
    : BP7_BlockProcessor(BundleProtocolVersion7::HOP_COUNT_BLOCK)
{
    log_path_ = "/dtn/bp7/hopcountblock";
}
...
int64_t
BP7_HopCountBlockProcessor::decode_block_data(uint8_t* data_buf, size_t data_len, Bundle* bundle)
{
    CborParser parser;
    CborValue cvElement;
    CborError err;

    std::string fld_name = "BP7_HopCountBlockProcessor::decode_block_data";
    ...
    int64_t status = validate_cbor_fixed_array_length(cvElement, 2, 2, num_elements, fld_name);
    CHECK_BP7_STATUS_RETURN

    uint64_t hop_limit = 0;
    uint64_t hop_count = 0;

    status = decode_uint(cvElement, hop_limit, fld_name);
    CHECK_BP7_STATUS_RETURN

    status = decode_uint(cvElement, hop_count, fld_name);
    CHECK_BP7_STATUS_RETURN

    // store the hop limmit and bump the hop count
    bundle->set_hop_limit(hop_limit);
    bundle->set_hop_count(hop_count + 1);
...
//     BP7_UNEXPECTED_EOF (1) = need more data to parse
int64_t
BP7_HopCountBlockProcessor::decode_cbor(SPtr_BlockInfo& block, uint8_t*  buf, size_t buflen)
{
    CborError err;
    CborParser parser;
    ...
    std::string fld_name = "BP7_HopCountBlockProcessor::decode_cbor";
...
//----------------------------------------------------------------------
ssize_t
BP7_HopCountBlockProcessor::consume(Bundle*    bundle,
                                   ...
                                   size_t     len)
{
    ...
    if (block->complete())
    {
        ...
        {
            // What to do here???
            log_warn_p(log_path(), "Error parsing Hop Count Block data - status = %" PRIi64, 
                       status);
        }
    }
...
//----------------------------------------------------------------------
int
BP7_HopCountBlockProcessor::prepare(const Bundle*    bundle,
                               ...
                               list_owner_t     list)
{
    ...
    {
        // do not include a hop count block if the hop limit is zero
...
//
int64_t
BP7_HopCountBlockProcessor::encode_block_data(uint8_t* buf, uint64_t buflen, 
                                           const Bundle* bundle, int64_t& encoded_len)
{
    CborEncoder encoder;
    CborEncoder dataArrayEncoder;
    CborError err;
    ...
    std::string fld_name = "BP7_HopCountBlockProcessor::encode_block_data";

    // Create the CBOR array fro the block
    err = cbor_encoder_create_array(&encoder, &dataArrayEncoder, 2);
    CHECK_CBOR_ENCODE_ERROR_RETURN

    // hop limit
    err = cbor_encode_uint(&dataArrayEncoder, bundle->hop_limit());
    CHECK_CBOR_ENCODE_ERROR_RETURN

    // hop limit
    err = cbor_encode_uint(&dataArrayEncoder, bundle->hop_count());
    CHECK_CBOR_ENCODE_ERROR_RETURN

    // close the array
    err = cbor_encoder_close_container(&encoder, &dataArrayEncoder);
    CHECK_CBOR_ENCODE_ERROR_RETURN
...
//
int64_t
BP7_HopCountBlockProcessor::encode_cbor(uint8_t* buf, uint64_t buflen, const SPtr_BlockInfo& block, const Bundle* bundle,
                                            uint8_t* data_buf, int64_t data_len, int64_t& encoded_len)
{
    ...
    std::string fld_name = "BP7_HopCountBlockProcessor::encode_cbor";
    ...
    if (block_first_cbor_byte != nullptr) {
        ...
        {
            log_crit_p(log_path(), "Error - First byte of Hop Count Block CBOR does not equal expected value (%2.2x): %2.2x", 
...
//----------------------------------------------------------------------
int
BP7_HopCountBlockProcessor::generate(const Bundle*  bundle,
                                SPtr_BlockInfoVec&  xmit_blocks,
                                SPtr_BlockInfo&     block,
                                const LinkRef& link,
                                bool           last)
{
    ...
    // the block data is a CBOR array with two elements - hop limit and hop count
    ...
    if (BP_FAIL == data_len)
    {
        log_err_p(log_path(), "Error encoding the Hop Count Block Data in 1st pass: *%p", bundle);
        return BP_FAIL;
    }
    ...
    {
        log_err_p(log_path(), "Error encoding the Hop Count Block Data in 2nd pass: *%p", bundle);
        return BP_FAIL;
    }
    ...
    {
        log_err_p(log_path(), "Error encoding Hop Count Block in 1st pass: *%p", bundle);
        return BP_FAIL;
    }
    ...
    if (BP_SUCCESS != status)
    {
        log_err_p(log_path(), "Error encoding Hop Count Block in 2nd pass: *%p", bundle);
        return BP_FAIL;
    }
...
//----------------------------------------------------------------------
int
BP7_HopCountBlockProcessor::format(oasys::StringBuffer* buf, BlockInfo *b)
{
    (void) b;
    return buf->append("HopCount");
}
