...
#ifndef _BUNDLE_LIST_INTMAP_H_
#define _BUNDLE_LIST_INTMAP_H_
 ...
 * by an unsigned int key. The expected purpose is to speed up finding
 * bundles by their bundle ID.
 *
 ...
 *
 */
class BundleListIntMap : public BundleListBase {
private:
    /**
     * Type for the list itself (private since it's irrelevant to the
     * outside).
     */
    typedef std::map<bundleid_t, Bundle*> List;
    typedef std::pair<bundleid_t, Bundle*> MPair;
...
public:
     ...
     * Constructor
     */
    BundleListIntMap(const std::string& name, oasys::SpinLock* lock = NULL,
                          const std::string& ltype="BundleListIntMap", 
                          const std::string& subpath="/listintmap/");

    /**
     * Destructor -- clears the list.
     */
    virtual ~BundleListIntMap();
     ...
     */
    virtual void insert(bundleid_t key, Bundle* bundle);

    virtual void insert(bundleid_t key, const BundleRef& bref) {
     ...
     *
     * @param used_notifier Popping off of the BundleListIntMap after coming 
     ...
     * list is empty.
     */
    virtual BundleRef pop_key(bundleid_t key, bool used_notifier = false);
     ...
     */
    virtual bool erase(bundleid_t key, bool used_notifier = false);
     ...
     * @return true if found, false if not
     */
    virtual bool contains(bundleid_t key) const;
     ...
     */
    virtual void move_contents(BundleListIntMap* other);
...
private:
     ...
     */
    virtual void add_bundle(Bundle* bundle, const bundleid_t key);
...
/**
 * A simple derivative to the BundleListIntMap class that hooks in an oasys
 ...
 */
class BlockingBundleListIntMap : public BundleListIntMap {
public:
    BlockingBundleListIntMap(const std::string& name, oasys::SpinLock* lock = NULL,
                       const std::string& ltype="BlockingBundleListIntMap", 
                       const std::string& subpath="/blockingliststrmultmap/");

    virtual ~BlockingBundleListIntMap();
