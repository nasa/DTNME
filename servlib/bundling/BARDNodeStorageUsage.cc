...
//----------------------------------------------------------------------
bool
BARDNodeStorageUsage::quota_type_src()
{
    return (quota_type_ == BARD_QUOTA_TYPE_SRC);
}
...
//----------------------------------------------------------------------
void
BARDNodeStorageUsage::check_email_frequency_period(size_t freq_period)
{
    ...
    if (freq_period > 0) {
        ...
        size_t elapsed = now.tv_sec - email_freq_period_start_;
        clear_flags = elapsed >= freq_period;

        if (clear_flags) {
            email_freq_period_start_ = now.tv_sec;
        }
    }

    if (clear_flags) {
        email_sent_internal_below_low_watermark_ = false;
        email_sent_internal_low_watermark_ = false;
        email_sent_internal_high_watermark_ = false;
        email_sent_internal_quota_reached_ = false;

        email_sent_external_below_low_watermark_ = false;
        email_sent_external_low_watermark_ = false;
        email_sent_external_high_watermark_ = false;
        email_sent_external_quota_reached_ = false;
    }
}
