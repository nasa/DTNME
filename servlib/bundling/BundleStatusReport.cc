...
{
    if (bundle->is_bpv6()) {
        BP6_BundleStatusReport::create_status_report(bundle, orig_bundle, sptr_source, status_flags, reason);
        return;
    }
                                  ...
                                  reason_t          reason)
{
    ...
    err = cbor_encode_uint(&rptArrayEncoder, static_cast<size_t>(reason));
    CHECK_CBOR_ENCODE_ERR_RETURN
                                             ...
                                             const bool include_timestamp)
{
    ...
    // 1) bundle received
    size_t indicated = (status_flags & STATUS_RECEIVED) ? 1 : 0;
...
//----------------------------------------------------------------------
int
BundleStatusReport::encode_indicator(CborEncoder& indicatorArrayEncoder, size_t indicated, 
                                             const bool include_timestamp)
{
    CborError err;
    CborEncoder indicatorEncoder;

    size_t timestamp = BundleTimestamp::get_current_time_millis();
...
BundleStatusReport::parse_status_report(CborValue& cvPayloadElements, CborUtilBP7& cborutil, data_t& sr_data)
{
    ...
    status = cborutil.decode_eid(cvStatusElements, sr_data.sptr_orig_source_eid_);
    CHECK_CBOR_STATUS
