...
#ifndef _BUNDLE_LIST_STRMULTIMAP_H_
#define _BUNDLE_LIST_STRMULTIMAP_H_
...
/**
 * MultiMap<string, Bundle*> structure for handling bundles  
 * by a string key with possible duplicates. The expected purpose is 
 * to speed up finding duplicate bundles.
 *
 ...
 *
 */
class BundleListStrMultiMap : public BundleListBase {
private:
    /**
     * Type for the list itself (private since it's irrelevant to the
     * outside).
     */
    typedef std::multimap<std::string, Bundle*> List;
...
public:
     ...
     * Constructor
     */
    BundleListStrMultiMap(const std::string& name, oasys::SpinLock* lock = NULL,
                          const std::string& ltype="BundleListStrMultiMap", 
                          const std::string& subpath="/liststrmultimap/");

    /**
     * Destructor -- clears the list.
     */
    virtual ~BundleListStrMultiMap();
     ...
     *
     * @param used_notifier Popping off of the BundleListStrMultiMap after coming 
     ...
     */
    virtual void move_contents(BundleListStrMultiMap* other);
...
/**
 * A simple derivative to the BundleListStrMultiMap class that hooks in an oasys
 ...
 */
class BlockingBundleListStrMultiMap : public BundleListStrMultiMap {
public:
    BlockingBundleListStrMultiMap(const std::string& name, oasys::SpinLock* lock = NULL,
                       const std::string& ltype="BlockingBundleListStrMultiMap", 
                       const std::string& subpath="/blockingliststrmultimap/");

    virtual ~BlockingBundleListStrMultiMap();
...
#endif /* _BUNDLE_LIST_STRMULTIMAP_H_ */
