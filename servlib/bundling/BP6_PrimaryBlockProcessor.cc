...
//----------------------------------------------------------------------
BP6_PrimaryBlockProcessor::BP6_PrimaryBlockProcessor()
    : BP6_BlockProcessor(BundleProtocolVersion6::PRIMARY_BLOCK)
{
	block_name_ = "BP6_Primary";
}

//----------------------------------------------------------------------
u_int64_t
BP6_PrimaryBlockProcessor::format_bundle_flags(const Bundle* bundle)
{
    u_int64_t flags = 0;
...
//----------------------------------------------------------------------
void
BP6_PrimaryBlockProcessor::parse_bundle_flags(Bundle* bundle, u_int64_t flags)
{
    ...
    if (flags & BUNDLE_SINGLETON_DESTINATION) {
        bundle->set_singleton_dest_flag(true);
    } else {
        bundle->set_singleton_dest_flag(false);
    }
...
//----------------------------------------------------------------------
u_int64_t
BP6_PrimaryBlockProcessor::format_cos_flags(const Bundle* b)
{
...
//----------------------------------------------------------------------
void
BP6_PrimaryBlockProcessor::parse_cos_flags(Bundle* b, u_int64_t cos_flags)
{
...
//----------------------------------------------------------------------
u_int64_t
BP6_PrimaryBlockProcessor::format_srr_flags(const Bundle* b)
{
...
//----------------------------------------------------------------------
void
BP6_PrimaryBlockProcessor::parse_srr_flags(Bundle* b, u_int64_t srr_flags)
{
...
//----------------------------------------------------------------------
size_t
BP6_PrimaryBlockProcessor::get_primary_len(const Bundle*  bundle,
                                       Dictionary*    dict,
                                       PrimaryBlock*  primary)
{
    ...
    if (bundle->custodian()->is_cbhe_compat() &&
        bundle->dest()->is_cbhe_compat() &&
        bundle->source()->is_cbhe_compat() &&
        bundle->replyto()->is_cbhe_compat())
    {
        primary->custodian_scheme_offset = bundle->custodian()->node_num();
        primary->custodian_ssp_offset = bundle->custodian()->service_num();

        primary->dest_scheme_offset = bundle->dest()->node_num();
        primary->dest_ssp_offset = bundle->dest()->service_num();

        primary->source_scheme_offset = bundle->source()->node_num();
        primary->source_ssp_offset = bundle->source()->service_num();

        primary->replyto_scheme_offset = bundle->replyto()->node_num();
        primary->replyto_ssp_offset = bundle->replyto()->service_num();
...
//----------------------------------------------------------------------
int
BP6_PrimaryBlockProcessor::prepare(const Bundle*    bundle,
                               ...
                               list_owner_t     list)
{
    ...
    xmit_blocks->insert(xmit_blocks->begin(), std::make_shared<BlockInfo>(this, source));
...
//----------------------------------------------------------------------
ssize_t
BP6_PrimaryBlockProcessor::consume(Bundle*    bundle,
                               BlockInfo* block,
                               u_char*    buf,
                               size_t     len)
{
     ...
     * BP6_BlockProcessor::consume() to get the default behavior (now that
    ...
    if (primary.version != BundleProtocolVersion6::CURRENT_VERSION) {
        log_warn_p(log, "protocol version mismatch %d != %d",
                   primary.version, BundleProtocolVersion6::CURRENT_VERSION);
        return -1;
    }
    bundle->set_bp_version(BundleProtocolVersion6::CURRENT_VERSION);
...
//                                            primary.creation_sequence));
 ...
 tooshort:
        log_err_p(log, "primary block advertised incorrect length %" PRIu64,
                  block->data_length());
        return -1;
    }
    ...
    if (primary.dictionary_length != 0)
    {
        ...
        if (buf[primary.dictionary_length - 1] != '\0') {
            log_err_p(log, "dictionary does not end with a nullptr character!");
            return -1;
        }
        ...
        SPtr_EID sptr_src_eid;
        dict->extract_eid(sptr_src_eid,
                          primary.source_scheme_offset,
                          primary.source_ssp_offset);
        bundle->set_source(sptr_src_eid);
    ...
    else
    {
        ...
        if (bundle->singleton_dest_flag()) {
            bundle->mutable_dest() = BD_MAKE_EID_IPN(primary.dest_scheme_offset,
                                                     primary.dest_ssp_offset);
        } else {
            bundle->mutable_dest() = BD_MAKE_EID_IMC(primary.dest_scheme_offset,
                                                     primary.dest_ssp_offset);
        }

        SPtr_EID sptr_src_eid = BD_MAKE_EID_IPN(primary.source_scheme_offset,
                                                primary.source_ssp_offset);
        bundle->set_source(sptr_src_eid);

        bundle->mutable_replyto() = BD_MAKE_EID_IPN(primary.replyto_scheme_offset,
                                                    primary.replyto_ssp_offset);

        bundle->mutable_custodian() = BD_MAKE_EID_IPN(primary.custodian_scheme_offset,
                                                      primary.custodian_ssp_offset);
    }
...
//----------------------------------------------------------------------
bool
BP6_PrimaryBlockProcessor::validate(const Bundle*           bundle,
...
//----------------------------------------------------------------------
int
BP6_PrimaryBlockProcessor::generate(const Bundle*  bundle,
                                BlockInfoVec*  xmit_blocks,
                                BlockInfo*     block,
                                const LinkRef& link,
                                bool           last)
{
    ...
    (void)block;
        ...
        //log_always_p(log_path(), "BP6_PrimaryBlockProcessor::generate");
...
//----------------------------------------------------------------------
void
BP6_PrimaryBlockProcessor::generate_primary(const Bundle* bundle,
                                        BlockInfoVec* xmit_blocks,
                                        BlockInfo*    block)
{
    static const char* log = "/dtn/bundle/protocol";
    ...
    *buf = BundleProtocolVersion6::CURRENT_VERSION;
    ++buf;
    --len;
...
#define PBP_WRITE_SDNV(value) { \
    ...
    // (values were filled in during the call to get_primary_len)
     ...
     */
    if (bundle->is_fragment()) {
        //log_debug_p(log,"BP6_PrimaryBlockProcessor::generate_primary: bundle is a fragment with frag_offset=%d orig_length=%d", bundle->frag_offset(), bundle->orig_length());
