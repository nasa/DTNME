...
BundleProtocolVersion6::prepare_blocks(Bundle* bundle, const LinkRef& link)
{
    ...
    const SPtr_BlockInfoVec& recv_blocks = bundle->recv_blocks();
    SPtr_BlockInfoVec& api_blocks  = bundle->api_blocks();
    BlockProcessor* bp;
    int i;
    ...
    if (recv_blocks->size() > 0) {
        ...
        //log_debug_p(LOG, "BundleProtocolVersion6::prepare_blocks: about to check that the first block is the primary block(it is of type=%d)", recv_blocks->front().type());
        ...
        //log_debug_p(LOG, "BundleProtocolVersion6::prepare_blocks: about to check whether this is a reactively fragmented bundle.");
            if (bundle->fragmented_incoming()
                && xmit_blocks->find_block(BundleProtocolVersion6::PAYLOAD_BLOCK)) {
                continue;
            }
...
//----------------------------------------------------------------------
size_t
BundleProtocolVersion6::total_length(Bundle* bundle, const SPtr_BlockInfoVec& blocks)
{
    (void) bundle;
    size_t ret = 0;
...
//----------------------------------------------------------------------
size_t
BundleProtocolVersion6::payload_offset(const SPtr_BlockInfoVec& blocks)
{
    size_t ret = 0;
    for (BlockInfoVec::const_iterator iter = blocks->begin();
         iter != blocks->end();
...
//----------------------------------------------------------------------
size_t
BundleProtocolVersion6::produce(const Bundle* bundle, const SPtr_BlockInfoVec& blocks,
                         ...
                         BundleProtocol::status_report_reason_t* deletion_reason)
{
    ...
    if (recv_blocks->front()->type() != BundleProtocolVersion6::PRIMARY_BLOCK) {
