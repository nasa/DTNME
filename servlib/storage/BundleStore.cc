...
BundleStore::del(Bundle* bundle)
{
    ...
    if (ret) {
        ...
        size_t durable_disk_size = cfg_.block_size_ * 
                                   ((bundle->durable_size() + cfg_.block_size_) / cfg_.block_size_);
        ASSERT(total_size_ >= bundle->durable_size());
        ASSERT(total_disk_size_ >= durable_disk_size);
        total_size_ -= bundle->durable_size();
        total_disk_size_ -= durable_disk_size;
...
BundleStore::del(bundleid_t bundleid, u_int64_t durable_size)
{
    ...
    if (ret) {
        ...
        size_t durable_disk_size = cfg_.block_size_ * 
                                   ((durable_size + cfg_.block_size_) / cfg_.block_size_);
...
BundleStore::reserve_payload_space(Bundle* bundle)
{
    if (!bundle->payload_space_reserved())
    {
        ...
        if (total_disk_size_ > max_disk_size_) {
            max_disk_size_ = total_disk_size_;
        }
...
BundleStore::try_reserve_payload_space(u_int64_t durable_size)
{
    ...
    if ((cfg_.payload_quota_ != 0) &&
        ((total_disk_size_ + durable_disk_size) > cfg_.payload_quota_))
    {
        result = false;
    }
    else
