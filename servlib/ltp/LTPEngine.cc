...
LTPNode::LTPNode(SPtr_LTPEngine ltp_engine_sptr, SPtr_LTPCLSenderIF clsender)
     : Logger("LTPNode", "/dtn/ltp/node/%lu", clsender->Remote_Engine_ID()),
       Thread("LTPNode")
{
...
LTPNode::Receiver::Receiver(LTPNode* parent_node, SPtr_LTPCLSenderIF clsender) :
      Logger("LTPNode::Receiver",
             "/dtn/ltp/node/%lu/rcvr", clsender->Remote_Engine_ID()),
      ...
      eventq_(logpath_)
{
    ...
    seg_processor_ = QPtr_RecvSegProcessor(new RecvSegProcessor(this, clsender_->Remote_Engine_ID()));
    seg_processor_->start();

    bundle_processor_ = QPtr_RecvBundleProcessor(new RecvBundleProcessor(this, clsender_->Remote_Engine_ID()));
...
LTPNode::Receiver::post_data(LTPDataReceivedEvent* event)
{
    ...
    int32_t recv_test = clsender_->Recv_Test();
        ...
        if (sptr_session && !sptr_session->Is_LTP_Cancelled())  {
            ...
            if (rs_seg != nullptr) {
                ...
                if (rs_seg->Retrans_Retries() <= retran_retries_) {
                    ...
                    clsender_->Send_Admin_Seg_Highest_Priority(rs_seg->asNewString(), timer, true);
                } else {
                    rs_seg->Set_Deleted();
        ...
        if (sptr_session != nullptr ) {
            ...
            if (cs_seg != nullptr) {
                ...
                if (cs_seg->Retrans_Retries() <= retran_retries_) {
                    ...
                    clsender_->Send_Admin_Seg_Highest_Priority(cs_seg->asNewString(), timer, false);
                } else {
                    cs_seg->Set_Deleted();
...
LTPNode::Receiver::dump_sessions(oasys::StringBuffer* buf)
{
    ...
    if (clsender_->Dump_Sessions() && (incoming_sessions_.size() > 0)) {
...
LTPNode::Receiver::build_CS_segment(LTPSession* session_ptr, int segment_type, u_char reason_code)
{
    ...
    if (!session_ptr->Is_LTP_Cancelled()) {
        ...
        clsender_->Send_Admin_Seg_Highest_Priority(cancel_segment->asNewString(), timer, false);
    }
...
LTPNode::Receiver::build_CS_segment(LTPSegment* seg_ptr, int segment_type, u_char reason_code)
{
    ...
    clsender_->Send_Admin_Seg_Highest_Priority(cancel_segment->asNewString(), dummy_timer, false);
}
...
{
    LTPCancelAckSegment* cas_seg = new LTPCancelAckSegment(seg_ptr, LTPSEGMENT_CAS_BS_BYTE, blank_session_.get()); 

    SPtr_LTPTimer dummy_timer;   // no retransmits
    clsender_->Send_Admin_Seg_Highest_Priority(cas_seg->asNewString(), dummy_timer, false);
    delete cas_seg;
    ...
    if (!closed) {
        ...
        if (sptr_session == nullptr) {
            if (create_flag) {
                ...
                sptr_session->Set_Inbound_Cipher_Suite(clsender_->Inbound_Cipher_Suite());
                sptr_session->Set_Inbound_Cipher_Key_Id(clsender_->Inbound_Cipher_Key_Id());
                sptr_session->Set_Inbound_Cipher_Engine(clsender_->Inbound_Cipher_Engine());
                sptr_session->Set_Outbound_Cipher_Suite(clsender_->Outbound_Cipher_Suite());
                sptr_session->Set_Outbound_Cipher_Key_Id(clsender_->Outbound_Cipher_Key_Id());
                sptr_session->Set_Outbound_Cipher_Engine(clsender_->Outbound_Cipher_Engine());

                if (clsender_->Use_Files_Recv()) {
                    bool run_with_disk_io_kludges = clsender_->Use_DiskIO_Kludge();

                    sptr_session->set_file_usage(clsender_->Dir_Path(), run_with_disk_io_kludges);
                }
    ...
    if (session_ptr->HaveReportSegmentsToSend(ds_seg, segsize, bytes_claimed))
    {
            ...
            iter++)
        {
            ...
            if (chkpt == rptseg->Checkpoint_ID())
            {
                ...
                clsender_->Send_Admin_Seg_Highest_Priority(rptseg->asNewString(), timer, true);
        ...
        if (seg_ptr->Segment_Type() == LTP_SEGMENT_CS_BS) {
            ...
            if (cancelled) {
                ...
                if (ds_seg->IsCheckpoint()) {
                    ...
                    SPtr_LTPTimer timer;
                    ++stats_.total_rs_segs_generated_;
                    clsender_->Send_Admin_Seg_Highest_Priority(sptr_rs->asNewString(), timer, true);
                    sptr_rs.reset();
                }
            }
        }
...
LTPNode::Receiver::process_incoming_data_segment(LTPSession* session_ptr, LTPSegment* seg_ptr)
{
    ...
    if (seg_ptr->Session_ID() > ULONG_MAX)
    {
        log_err("%s: received %zu:%zu DS with invalid Session ID - exceeds ULONG_MAX", 
                  __func__, session_ptr->Engine_ID(), session_ptr->Session_ID());
...
LTPNode::Receiver::RecvBundleProcessor::shutdown()
{
    ...
    if (eventq_.size() > 0) {
        log_always("LTPEngine::Receiver(%zu) - Extracting bundles from %zu received sessions before shutting down",
                   parent_rcvr_->Remote_Engine_ID(), eventq_.size());
    }
    lock_.unlock();
            ...
            if (cc < 0)  {
                ...
                if (bundle_complete) {
                    ...
                    if (! bdaemon->query_accept_bundle_based_on_quotas(bundle, payload_space_reserved, 
                        ...
                        }  else if (bundle->is_admin() && bundle->is_bpv7() && 
                                    is_bibe_with_custody_transfer(bundle)) {
                            // safe to delete this bundle
                            delete bundle;
                            bundle = nullptr;
                            bundle_deleted = true;
                    ...
                    if (!bundle_deleted) {
                        ...
                        BundleDaemon::post( new BundleReceivedEvent(bundle, EVENTSRC_PEER, 
                                                                    bundle_on_wire_len, 
                                                                    EndpointID::NULL_EID(), nullptr) );
                        bundle = nullptr;
                    }
                }
            }
...
LTPNode::Sender::Sender(SPtr_LTPEngine ltp_engine_sptr, LTPNode* parent_node, SPtr_LTPCLSenderIF clsender)
    : 
      Logger("LTPNode::Sender", "/dtn/ltp/node/%zu/sender", clsender->Remote_Engine_ID()),
      ...
      parent_node_(parent_node)
{
    ...
    bundle_processor_ = QPtr_SndrBundleProcessor(new SndrBundleProcessor(this, clsender_->Remote_Engine_ID()));
    bundle_processor_->start();
...
LTPNode::Sender::PostTransmitProcessing(LTPSession* session_ptr, bool success)
{
    ...
    while ((bref = session_ptr->Bundle_List()->pop_back()) != nullptr)
    {
        if (bref->expired_in_link_queue()) {
            ...
            {
                clsender_->PostTransmitProcessing(bref, true,  session_ptr->Expected_Red_Bytes(), success);
            } else if (session_ptr->Expected_Green_Bytes() > 0) { 
                clsender_->PostTransmitProcessing(bref, false,  session_ptr->Expected_Green_Bytes(), success);
            } else {
...
LTPNode::Sender::process_RS(LTPReportSegment* rpt_seg, bool closed, bool cancelled)
{
    ...
    if (closed) {
        ...
        if (!cancelled) {
            // send RS ACK
            LTPReportAckSegment* ras = new LTPReportAckSegment(rpt_seg, blank_session_.get());

            ++stats_.total_snt_ra_;

            SPtr_LTPTimer dummy_timer;
            clsender_->Send_Admin_Seg_Highest_Priority(ras->asNewString(), dummy_timer, false);
            delete ras;
    ...
    SPtr_LTPTimer dummy_timer;
    clsender_->Send_Admin_Seg_Highest_Priority(ras->asNewString(), dummy_timer, true);
    delete ras;
    ...
    if (retransmit_dsmap_ptr->size() > 0)
    {
        ...
        while (iter != retransmit_dsmap_ptr->end())
        {
            ...
            if (!sptr_ds_seg->Queued_To_Send()) {
                ...
                clsender_->Send_DataSeg_Higher_Priority(sptr_ds_seg, sptr_timer);
            }
...
LTPNode::Sender::process_CS(LTPCancelSegment* cs_segment, bool closed, bool cancelled)
{
    ...
    SPtr_LTPTimer dummy_timer;
    clsender_->Send_Admin_Seg_Highest_Priority(cas_seg->asNewString(), dummy_timer, true);
    delete cas_seg;
...
LTPNode::Sender::dump_sessions(oasys::StringBuffer* buf)
{
    ...
    if (clsender_->Dump_Sessions() && (send_sessions_.size() > 0)) {
...
LTPNode::Sender::send_bundle(const BundleRef& bref)
{
    ...
    do {  // just to limit the scope of the lock
        ...
        SPtr_BlockInfoVec sptr_blocks = clsender_->GetBlockInfoVec(bundle_ptr);
    ...
    if (green) {
        ...
        update_session_counts(loading_session_.get(), LTPSession::LTP_SESSION_STATE_DS); 
...
LTPNode::Sender::process_bundles(LTPSession* session_ptr)
{
    ...
    if (clsender_->Use_Files_Xmit() && session_ptr->Session_Size() >= 10000000) {
    ...
    size_t bytes_per_checkpoint = clsender_->Bytes_Per_CheckPoint();
    ...
    while (!start_shutting_down_ && (bundle_iter != session_ptr->Bundle_List()->end()))
    {
        ...
        SPtr_BlockInfoVec sptr_blocks = clsender_->GetBlockInfoVec(bundle);
        ...
        clsender_->Delete_Xmit_Blocks(bref);
                                    ...
                                    bool is_checkpoint, bool last_seg, bool force_send_seg)
{
    ...
    if (!sptr_ds_seg->Queued_To_Send()) {
        sptr_ds_seg->Set_Queued_To_Send(true);
        clsender_->Send_DataSeg_Low_Priority(sptr_ds_seg, retransmit_timer);
        ds_sent = true;
    }
...
LTPNode::Sender::process_bundles_using_a_file(LTPSession* session_ptr)
{
    ...
    std::string file_path;
    session_ptr->set_file_usage(clsender_->Dir_Path());
        ...
        ++bundle_iter)
    {
        ...
        if (bundle->expired() || bundle->manually_deleting())
        {
            ...
            if (session_ptr->Send_Red()) {
                ...
                if (!clsender_->Del_From_InFlight_Queue(bref)) {
                ...
                if (sptr_ds_seg != nullptr) {
                    ...
                    if (sptr_ds_seg->Queued_To_Send()) {
                        ...
                        if (sptr_ds_seg->Retrans_Retries() <= retran_retries_) {
                            ...
                            clsender_->Send_DataSeg_Higher_Priority(sptr_ds_seg, timer);
