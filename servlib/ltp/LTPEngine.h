...
public:
        ...
        SPtr_LTPCLSenderIF CLSender()  { return sptr_clsender_;  }
        SPtr_LTPNode       LTP_Node()  { return sptr_ltpnode_;   }

        void Set_LTPNode(SPtr_LTPNode sptr_node) { sptr_ltpnode_ = sptr_node; }

private:

        uint64_t           remote_engine_id_; ///< will hold engine id
        SPtr_LTPCLSenderIF sptr_clsender_;    ///< will hold IF to get back to clsender. 
                                              ///<     (has a shared_ptr to the LTPCL::Sender)
        SPtr_LTPNode       sptr_ltpnode_;     ///< will get created if not already running.
};
...
public:
    ...
    virtual bool register_engine(SPtr_LTPCLSenderIF sptr_clsender);
...
private:
                             ...
                             public oasys::Thread
    {
        ...
        protected:
            ...
            meutils::MsgQueue< struct LTPDataReceivedEvent* > me_eventq_;
...
public:
     ...
     * Constructor.
     */
    LTPNode(SPtr_LTPEngine sptr_ltp_engine_sptr, SPtr_LTPCLSenderIF  sptr_cl_sender);
     ...
     */ 
    SPtr_LTPCLSenderIF CLSender() { return sptr_clsender_; }
    uint64_t Remote_Engine_ID() { return sptr_clsender_->Remote_Engine_ID(); }
    ...
    void Post_Timer_To_Process(oasys::SPtr_Timer sptr_event);
...
protected:
    ...
    public:
         ...
         * Constructor.
         */
        Receiver(LTPNode* parent_node, SPtr_LTPCLSenderIF sptr_clsender);
         ...
         */
        void     RSIF_post_timer_to_process(oasys::SPtr_Timer sptr_event) override;
        ...
        uint64_t ltp_queued_bytes_quota() { return sptr_clsender_->Ltp_Queued_Bytes_Quota(); }

        bool keep_aborted_files() { return sptr_clsender_->Keep_Aborted_Files(); }
    ...
    protected:
        ...
        public:
            ...
            virtual void post(SPtr_LTP_DS_MAP sptr_seg_list, size_t len, bool multi_bundle);
        ...
        protected:
            ...
            meutils::MsgQueue< struct BundleProcessorEventObj* > me_eventq_;
    ...
    protected:
        ...
        SPtr_LTPCLSenderIF sptr_clsender_;
        ...
        SPtr_LTPSession sptr_blank_session_;
        /// Stats instance
        Stats stats_;
...
protected:
    ...
    public:
         ...
         * Constructor.
         */
        Sender(SPtr_LTPEngine ltp_engine_sptr, LTPNode* parent_node, SPtr_LTPCLSenderIF sptr_clsender);
    ...
    protected:
        ...
        protected:
            ...
            meutils::MsgQueue< struct EventObj* > me_eventq_;
    ...
    protected:
        ...
        SPtr_LTPSession sptr_blank_session_;
        SPtr_LTPSession sptr_loading_session_;

        // local copies of the CLSender parameter values
        uint32_t max_sessions_        = 100;
        uint32_t agg_time_millis_     = 500;      // millisecs
        ...
        // flag to signal that an LTP session is ready to send
        std::mutex              agg_time_lock_;
        std::condition_variable cond_var_agg_time_changed_;
    };   
    ...
    public:
        ...
        virtual void run();
        virtual void post(oasys::SPtr_Timer sptr_event);
    protected:
        /// Message queue for accepting BundleEvents from ExternalRouter
        meutils::MsgQueue<oasys::SPtr_Timer> me_eventq_;
