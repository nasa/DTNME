...
#include <third_party/oasys/thread/Lock.h>
    ...
    : TclCommand("link")
{
    ...
    "            qlimit_enabled <true or falsie  (default false)>\n"
    ...
    add_to_help("options [<cla_name>]", 
                "list available Convergence Layer Adapters or valid options for a specified CLA\n");
...
LinkCommand::exec(int argc, const char** argv, Tcl_Interp* interp)
{
    ...
    if (strcmp(cmd, "add") == 0) {
        ...
        if (set_available) {
            ...
            LinkStateChangeRequest* event_to_post;
            event_to_post = new LinkStateChangeRequest(link, Link::AVAILABLE,
                                                       ContactEvent::USER);
            SPtr_BundleEvent sptr_event_to_post(event_to_post);
            BundleDaemon::post(sptr_event_to_post);
    ...
    else if (strcmp(cmd, "options") == 0) 
    {
        ...
        if (argc == 3) {
            ...
            CLVector* cla_list = CLVector::instance();
            CLVector::iterator iter = cla_list->begin();
            while (iter != cla_list->end()) {
                ConvergenceLayer* cla = (*iter);

                if (0 == strcmp(name, cla->name())) {
                    oasys::StringBuffer buf;
                    cla->list_link_opts(buf);
                    set_result(buf.c_str());
                    return TCL_OK;
                }
                ++iter;
            ...
            // output err and fall through to provide alist of valid CLAs
            append_resultf("CLA not found: %s\n\n", name);
        }

        append_resultf("Available Convergence Layer Adapters:\n");
        ...
        append_resultf("Use the command \"link options <cla_name> to view valid options for that cls_name\n");
    ...
    else if (strcmp(cmd, "dump") == 0) 
    {
        ...
        if (argc == 2) {
                 ...
                 i != links->end(); ++i)
            {
                ...
                linkptr = (*i).object();
                if (0 == linkptr->lock()->try_lock(__func__)) {
                    linkptr->get_cla_stats(buf);

                    append_resultf("*%p  - bundles queued: %zu in-flight: %zu %s\n",
                                   linkptr, linkptr->bundles_queued(),
                                   linkptr->bundles_inflight(), buf.c_str());

                    linkptr->lock()->unlock();
                } else {
                    append_resultf("*%p  - <unable to get stats - try again later>\n", linkptr);
                }
            }
            links = cm->previous_links();
            append_resultf("\nPreviously active links:\n");
