...
ContactCommand::exec(int argc, const char** argv, Tcl_Interp* interp)
{
    ...
    if(strcmp(argv[1], "load") == 0) {
          ...
          SPtr_EID sptr_eid = BundleDaemon::instance()->local_eid_ipn();
          buf.append("\nPlan Loaded for " + sptr_eid->str() + "\n");
          buf.append("Format: <Contact ID> <EID> <Link Name> <Start Time> <Duration>\n");
          std::vector<dtn::ContactPlanner::Contact_Entry> tmpList;
          tmpList = ContactPlanner::instance()->get_plan(sptr_eid);
          for (std::vector<dtn::ContactPlanner::Contact_Entry>::iterator it = tmpList.begin(); it != tmpList.end(); ++it)
          {
            ...
            std::string eid = it->sptr_eid_->str();
          ...
          SPtr_EID sptr_eid = BD_MAKE_EID(argv[3]);
          bool export_plan = ContactPlanner::instance()->export_plan(sptr_eid, argv[4]);
          if(export_plan) {
          ...
          SPtr_EID sptr_eid = BD_MAKE_EID(argv[3]);
          ContactPlanner::instance()->add_contact(sptr_eid, argv[4], argv[5], argv[6]);
          oasys::StringBuffer buf;
          ...
          SPtr_EID sptr_eid = BD_MAKE_EID(argv[3]);
          ContactPlanner::instance()->delete_contact(sptr_eid, argv[4]);
          oasys::StringBuffer buf;
          std::string eid(argv[3]);
          buf.append("\nUpdated Plan for "+eid+"\n");
          buf.append("Format: <Contact ID> <EID> <Link Name> <Start Time> <Duration>\n");
      ...
      if (strcmp(argv[2], "plan") == 0) {
          ...
          buf.append("Format: <Contact ID> <EID> <Link Name> <Start Time> <Duration>\n");
          std::vector<dtn::ContactPlanner::Contact_Entry> tmpList;
          tmpList = ContactPlanner::instance()->get_contact(sptr_eid, argv[4]);
          for (std::vector<dtn::ContactPlanner::Contact_Entry>::iterator it = tmpList.begin(); it != tmpList.end(); ++it)
          {
            std::string contact_id = std::to_string(it->id_);
            std::string eid = it->sptr_eid_->str();
