    ...
    : TclCommand("bard") 
{
    ...
    add_to_help("unlimited_quota <quota type> <naming scheme> <node number/name>",
                "clear limits of a node quota entry and leaves it in the database; where:\n"
                "        <quota type>           - 'dst' = destination node or 'src' = source node\n"
                "        <naming scheme>        - 'ipn', 'imc' or 'dtn'\n"
                "        <node number/name>     - 'ipn' and 'imc' require a number and 'dtn' requires a name\n"
    ...
    add_to_help("committed [exact]", "list (usage + reserved) data for all nodes with and without quotas using magnitude values or exact values if specified");
    ...
    bind_var(new oasys::BoolOpt("email_enabled",
                                &BundleArchitecturalRestagingDaemon::params_.email_notifications_enabled_,
                                "Enable or disable sending email notifications when bundle quota watermarks are triggered "
                                "(default is true)"));

    bind_var(new oasys::SizeOpt("email_frequency",
                                &BundleArchitecturalRestagingDaemon::params_.email_frequency_,
                                "seconds",
                                "Limit each quota related message type to once per frequency seconds time period.\n"
                                "        This may cause some messages to appear out of order since some may be skipped.\n"
                                "        (default is 600 seconds)"));

    bind_var(new oasys::BoolOpt("treat_unknown_link_as_pooled",
                                &BundleArchitecturalRestagingDaemon::params_.treat_unknown_link_as_pooled_,
                                "If quota references unknown link name then use pooled external storage "
                                "(default is true)"));

    add_to_help("internal_low_watermark [<new_low_watermark>]",
                "view or set the internal storage low watermark percentage for email notifications (default is 25%)\n"
                );

    add_to_help("internal_high_watermark [<new_high_watermark>]",
                "view or set the internal storage high watermark percentage for email notifications (default is 75%)\n"
                );

    add_to_help("external_low_watermark [<new_low_watermark>]",
                "view or set the external storage low watermark percentage for email notifications (default is 25%)\n"
                );

    add_to_help("external_high_watermark [<new_high_watermark>]",
                "view or set the external storage high watermark percentage for email notifications (default is 75%)\n"
                );
...
BundleRestagingCommand::exec(int argc, const char** argv, Tcl_Interp* interp)
{
    ...
    } else if (0 == strcmp(cmd, "committed")) {
        return process_committed(argc, argv);
    ...
    } else if (0 == strcmp(cmd, "internal_low_watermark")) {
        return process_internal_low_watermark(argc, argv);

    } else if (0 == strcmp(cmd, "internal_high_watermark")) {
        return process_internal_high_watermark(argc, argv);

    } else if (0 == strcmp(cmd, "external_low_watermark")) {
        return process_external_low_watermark(argc, argv);

    } else if (0 == strcmp(cmd, "external_high_watermark")) {
        return process_external_high_watermark(argc, argv);
...
//----------------------------------------------------------------------
int
BundleRestagingCommand::process_committed(int argc, const char** argv)
{
    if (argc > 3) {
        wrong_num_args(argc, argv, 2, 2, 3);
        return TCL_ERROR;
    ...
    if (exact) {
        sptr_bard->bardcmd_committed_exact(buf);
    } else {
        sptr_bard->bardcmd_committed(buf);
    }
   ...
   //  add_to_help("reload <quota type> <naming scheme> <node number/name> [<new expiration secs> <new dest EID>]",
    ...
    while (++idx < argc) {
        if ((*argv[idx] == 'i') || (*argv[idx] == 'd')) {
            SPtr_EID sptr_eid = BD_MAKE_EID(argv[idx]);

            bool is_valid = sptr_eid->valid();
...
//----------------------------------------------------------------------
int
BundleRestagingCommand::process_internal_high_watermark(int argc, const char** argv)
{
    //  add_to_help("internal_high_watermark [<new_high_watermark>]",
    //              "view or set the high watermark percentage for email notifications (default is 75%)\n"
    ...
    if (argc == 2) {
        if (!sptr_bard->bardcmd_get_internal_high_watermark(result_str)) {
            resultf("%s", result_str.c_str());
            return TCL_ERROR;
        }

    } else if (argc > 2) {
        if (!parse_number_value(argv[idx], new_watermark)) {
            resultf("invalid new internal storage high watermark: %s", argv[idx]);
            return TCL_ERROR;
        }

        if (new_watermark >= 100) {
            resultf("invalid new internal storage high watermark: %s (0 to 99 is valid)", argv[idx]);
            return TCL_ERROR;
        }

        if (!sptr_bard->bardcmd_set_internal_high_watermark(new_watermark, result_str)) {
            resultf("%s", result_str.c_str());
            return TCL_ERROR;
        }
    }
...
//----------------------------------------------------------------------
int
BundleRestagingCommand::process_internal_low_watermark(int argc, const char** argv)
{
    //  add_to_help("internal_low_watermark [<new_low_watermark>]",
    //              "view or set the low watermark percentage for email notifications (default is 25%)\n"
    ...
    if (argc == 2) {
        if (!sptr_bard->bardcmd_get_internal_low_watermark(result_str)) {
            resultf("%s", result_str.c_str());
            return TCL_ERROR;
        }
        ...
        if (!parse_number_value(argv[idx], new_watermark)) {
            resultf("invalid new internal storage low watermark: %s", argv[idx]);
            return TCL_ERROR;
        }

        if (new_watermark >= 100) {
            resultf("invalid new internal storage low watermark: %s (0 to 99 is valid)", argv[idx]);
            return TCL_ERROR;
        }

        if (!sptr_bard->bardcmd_set_internal_low_watermark(new_watermark, result_str)) {
            resultf("%s", result_str.c_str());
            return TCL_ERROR;
        }
    }
...
//----------------------------------------------------------------------
int
BundleRestagingCommand::process_external_high_watermark(int argc, const char** argv)
{
    //  add_to_help("external_high_watermark [<new_high_watermark>]",
    //              "view or set the high watermark percentage for email notifications (default is 75%)\n"
    ...
    if (argc == 2) {
        if (!sptr_bard->bardcmd_get_external_high_watermark(result_str)) {
            resultf("%s", result_str.c_str());
            return TCL_ERROR;
        }
        ...
        if (!parse_number_value(argv[idx], new_watermark)) {
            resultf("invalid new external storage high watermark: %s", argv[idx]);
            return TCL_ERROR;
        }

        if (new_watermark >= 100) {
            resultf("invalid new external storage high watermark: %s (0 to 99 is valid)", argv[idx]);
            return TCL_ERROR;
        }

        if (!sptr_bard->bardcmd_set_external_high_watermark(new_watermark, result_str)) {
            resultf("%s", result_str.c_str());
            return TCL_ERROR;
        }
    }
...
//----------------------------------------------------------------------
int
BundleRestagingCommand::process_external_low_watermark(int argc, const char** argv)
{
    //  add_to_help("low_watermark [<new_low_watermark>]",
    //              "view or set the low watermark percentage for email notifications (default is 25%)\n"
    ...
    if (argc == 2) {
        if (!sptr_bard->bardcmd_get_external_low_watermark(result_str)) {
            resultf("%s", result_str.c_str());
            return TCL_ERROR;
        }
        ...
        if (!parse_number_value(argv[idx], new_watermark)) {
            resultf("invalid new external storage low watermark: %s", argv[idx]);
            return TCL_ERROR;
        }

        if (new_watermark >= 100) {
            resultf("invalid new external storage low watermark: %s (0 to 99 is valid)", argv[idx]);
            return TCL_ERROR;
        }

        if (!sptr_bard->bardcmd_set_external_low_watermark(new_watermark, result_str)) {
            resultf("%s", result_str.c_str());
            return TCL_ERROR;
        }
    }
