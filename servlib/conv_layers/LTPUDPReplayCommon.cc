...
LTPUDPSession::Start_Inactivity_Timer(time_t time_left)
{
    ...
    inactivity_timer_ = std::make_shared<InactivityTimer>(parent_, session_key_,  parent_type_);
...
InactivityTimer::start()
{
    ...
    if (sptr_ != nullptr) {
        SetSeconds(seconds_);
        oasys::SPtr_Timer timer = sptr_;
        oasys::SharedTimer::schedule_in(seconds_*1000 + 100, timer);
    }
}
...
InactivityTimer::cancel()
{
    ...
    if (sptr_ != nullptr) {
        oasys::SPtr_Timer timer = sptr_;
        result = oasys::SharedTimer::cancel_timer(timer);
...
//----------------------------------------------------------------------    
void
InactivityTimer::timeout(const struct timeval &now)
{
    (void)now;
    ...
    if (cancelled() || sptr_ == nullptr) {
        ...
        first_call_ = false;
        parent_->Post_Timer_To_Process(sptr_);
        ...
        if (right_now >= target_counter_)
        {
            ...
            schedule_in(seconds*1000, timer);
        }
    }
}
...
//--------------------------------------------------------------------------
void
LTPUDPSegment::Create_RS_Timer()
{
    //log_crit("%s:- not used in replay - unexpected call!!!  %s", __func__, session_key_.c_str());
}

//--------------------------------------------------------------------------
void
LTPUDPSegment::Start_Retransmission_Timer()
{
    //log_crit("%s:- not used in replay - unexpected call!!!  %s", __func__, session_key_.c_str());
}
...
//--------------------------------------------------------------------------
void
LTPUDPSegment::Create_Retransmission_Timer()
{
    //log_crit("%s:- not used in replay - unexpected call!!!  %s", __func__, session_key_.c_str());
}
...
LTPUDPCancelSegment::LTPUDPCancelSegment(LTPUDPCancelSegment& from)  : LTPUDPSegment(from)
{
    ...
    packet_len_   = from.packet_len_;
    // Cancel Segment is not used in replay and this triggers a waring in newer compilers
