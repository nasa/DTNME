...
LTPUDPConvergenceLayer::list_link_opts(oasys::StringBuffer& buf)
{
    ...
    buf.append("    agg_size <U64>                     - accumulate bundles for an LTP session until agg_size bytes of payload is reached (default: 1000000)\n");
    ...
    buf.append("    ccsds_compatible <Bool>            - whether to use CCSDS 734.1-B-1 compatibility which specifies client service ID 2 for Data Aggregation \n");
    buf.append("                                            (default: false  which is compatible with ION through at least 3.7.3 / 4.0.2)\n");
                                                       ...
                                                       uint64_t expected_bytes,  bool success)
{
    ...
    if (expected_bytes > 0) { 
        ...
        if (isred) {
            ...
            event_to_post = new BundleTransmittedEvent(bref.object(), contact_, 
                                                       link, expected_bytes,
      ...
      contact_(contact.object(), "LTPUDPCovergenceLayer::LTPUDPSender")
{
    ...
    // create shared mutex and condition var to be used by all of the MsgQueues
    sptr_eventq_lock_ = std::make_shared<std::mutex>();
    sptr_cond_var_eventq_ = std::make_shared<std::condition_variable>();

    qptr_admin_eventq_= QPtr_MsgQueueX(new meutils::MsgQueueX<MySendObject*>(sptr_eventq_lock_, sptr_cond_var_eventq_));
    qptr_ds_high_eventq_= QPtr_MsgQueueX(new meutils::MsgQueueX<MySendObject*>(sptr_eventq_lock_, sptr_cond_var_eventq_));
    qptr_ds_low_eventq_= QPtr_MsgQueueX(new meutils::MsgQueueX<MySendObject*>(sptr_eventq_lock_, sptr_cond_var_eventq_));
...
LTPUDPConvergenceLayer::LTPUDPSender::shutdown()
{
    ...
    if (params_->comm_aos_) {
        while (qptr_admin_eventq_->size() > 0) {
            usleep(100000);
        }
    }
    ...
    MySendObject *event;
    while (qptr_admin_eventq_->try_pop(&event)) {
        delete event->str_data_;
        delete event;
    }

    while (qptr_ds_high_eventq_->try_pop(&event)) {
        delete event->str_data_;
        delete event;
    }

    while (qptr_ds_low_eventq_->try_pop(&event)) {
        delete event->str_data_;
        delete event;
    }
}
...
LTPUDPConvergenceLayer::LTPUDPSender::run()
{
    ...
    std::unique_lock<std::mutex> qlok(eventq_size_lock_, std::defer_lock);

    const char* event_priority = nullptr;
    bool okay_to_send = true;

    bool event_was_queued = true;
    ...
    while (!should_stop()) {
        ...
        if (!got_event) {
            // waiting on one  of the eventqs should trigger if any of them have an event pushed
            event_was_queued = qptr_admin_eventq_->wait_for_millisecs(100); // wait up to 1/10th second

            if (!event_was_queued) {
                if (++num_timeouts >= 5) {
                    // to make sure an event did not get missed - check if 5 timeouts in a row
                    event_was_queued = true;
                    num_timeouts = 0;
                }
            } else {
                num_timeouts = 0;
        ...
        if (event_was_queued) {
            if (qptr_admin_eventq_->try_pop(&event)) {
                event_priority = "high priority admin";
                got_event = true;
            } else if (qptr_ds_high_eventq_->try_pop(&event)) {
                event_priority = "higher priority DS resend";
                got_event = true;
            } else if (qptr_ds_low_eventq_->try_pop(&event)) {
        ...
        if (got_event) {
            ...
            if (event->str_data_) {
                ...
                } while (false); // just limiting the scope of the lock

                // don't try to start the timer until releasing the lock on the data segment
                // to prevent deadly embrace while shutting down
                if ( do_start_timer)
                {
                    if (event->timer_) {
                        event->timer_->start();
...
LTPUDPConvergenceLayer::LTPUDPSender::Send_Admin_Seg_Highest_Priority(std::string * send_data, SPtr_LTPTimer timer, bool back)
{
    if (!should_stop()) {
        ...
        // add to size and release lock before pushing 
        if (true) {
            std::lock_guard<std::mutex> scoplok(eventq_size_lock_);
        ...
        (void) back;
        qptr_admin_eventq_->push_back(obj);
    } else {
        delete send_data;
    }
}
...
LTPUDPConvergenceLayer::LTPUDPSender::Send_DataSeg_Higher_Priority(SPtr_LTPDataSegment sptr_ds_seg, SPtr_LTPTimer timer)
{
    if (!should_stop()) {
        ...
        qptr_ds_high_eventq_->push_back(obj);
    }
}
...
LTPUDPConvergenceLayer::LTPUDPSender::Send_DataSeg_Low_Priority(SPtr_LTPDataSegment sptr_ds_seg, SPtr_LTPTimer timer)
{
    if (!should_stop()) {
        ...
        qptr_ds_low_eventq_->push_back(obj);
    }
}
...
LTPUDPConvergenceLayer::LTPUDPSender::dump_stats(oasys::StringBuffer* buf)
{
    ...
    buf->appendf("\nTransmit queues - admin: %zu  resend DS: %zu  send DS: %zu   Total bytes: %zu (%s) Max: %zu (%s)\n\n", 
                 qptr_admin_eventq_->size(), qptr_ds_high_eventq_->size(), qptr_ds_low_eventq_->size(), 
...
LTPUDPConvergenceLayer::LTPUDPSender::SendPoller::run()
{
    ...
    while (!should_stop()) {
        if (!parent_->check_ready_for_bundle()) {
            // sleep 1/100th sec
            dummy_cond_var.wait_for(qlok, std::chrono::milliseconds(10));
        }
    }
}
