...
BIBEConvergenceLayer::BIBE::encapsulate_bundle(BundleRef& bref)
{
    ...
    bool accept_bundle = BundleDaemon::instance()->query_accept_bundle_based_on_quotas(bibe_bundle, space_reserved, prev_reserved_space);
    while (!should_stop() && !accept_bundle) {
        usleep(100000);
        accept_bundle = BundleDaemon::instance()->query_accept_bundle_based_on_quotas(bibe_bundle, space_reserved, prev_reserved_space);
    }
