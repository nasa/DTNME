                                ...
                                int argc, const char* argv[])
{
    ...
    SPtr_EID sptr_eid = BD_MAKE_EID(link->nexthop());
    if (! sptr_eid->valid()) {
    ...
    if (params->src_eid_.empty()) {
        ...
        {
            params->src_eid_ = bd->local_eid()->str();
        } else {
            params->src_eid_ = bd->local_eid_ipn()->str();
        }
    }
    sptr_eid = BD_MAKE_EID(params->src_eid_);
    if (! sptr_eid->valid()) {
...
BIBEConvergenceLayer::BIBE::encapsulate_bundle(BundleRef& bref)
{
    ...
    SPtr_BlockInfoVec sptr_blocks = bref->xmit_blocks()->find_blocks(contact_->link());
    ...
    if (sptr_blocks == nullptr) {
        ...
        BundleSendCancelledEvent* event_to_post;
        event_to_post = new BundleSendCancelledEvent(bref.object(), link_);
        SPtr_BundleEvent sptr_event_to_post(event_to_post);
        BundleDaemon::post(sptr_event_to_post);
    ...
    uint64_t formatted_len = BundleProtocol::total_length(bref.object(), sptr_blocks.get());
    ...
    bibe_bundle->mutable_dest() = BD_MAKE_EID(params_->dest_eid_);
    bibe_bundle->mutable_replyto() = BD_MAKE_EID_NULL();
    bibe_bundle->mutable_custodian() = BD_MAKE_EID_NULL();
        ...
        if (bibe_bundle->is_bpv6()) {
            ...
            if (bref->local_custody()) {
                if (bref->custody_dest() != params_->dest_eid_) {
                    bdaemon->switch_bp6_to_bibe_custody(bref.object(), params_->dest_eid_);
                }
            } else {
                bdaemon->accept_bibe_custody(bref.object(), params_->dest_eid_);
            }
        }
    }
    ...
    while (formatted_len > 0) {
        ...
        ret = BundleProtocol::produce(bref.object(), sptr_blocks.get(), scratch_.end(), 
    ...
    bool accept_bundle = bdaemon->query_accept_bundle_based_on_quotas(bibe_bundle, 
                                                                      space_reserved,
                                                                      prev_reserved_space);
    while (!should_stop() && !accept_bundle) {
        usleep(100000);
        accept_bundle = bdaemon->query_accept_bundle_based_on_quotas(bibe_bundle,
                                                                     space_reserved,
                                                                     prev_reserved_space);
    }
    ...
    if (true) {  // for scope
        ...
        event_to_post = new BundleReceivedEvent(bibe_bundle, EVENTSRC_ADMIN, sptr_dummy_prevhop);
        SPtr_BundleEvent sptr_event_to_post(event_to_post);
        BundleDaemon::post(sptr_event_to_post);
    }
    ...
    bref->clear_redirect_orig_link(link_->name_str());

    if (true) {  // for scope
        BundleTransmittedEvent* event_to_post;
        event_to_post = new BundleTransmittedEvent(bref.object(), contact_, link_, encoded_len, 
                                                   encoded_len, true, true);
        SPtr_BundleEvent sptr_event_to_post(event_to_post);
        BundleDaemon::post(sptr_event_to_post);
    }

    if (params_->bp_version_ == 6) {
        bdaemon->inc_bibe6_encapsulations();
    } else {
        bdaemon->inc_bibe7_encapsulations();
    }
}
