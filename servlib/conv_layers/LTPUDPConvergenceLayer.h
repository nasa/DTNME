...
public:
    ...
    public:
        ...
        uint64_t   ltp_queued_bytes_quota_   = 2000000000;     ///< max LTP bytes to queue for bundle extraction before cancelling sessions
...
protected:
    ...
    public:
         ...
         * Constructor.
         */
        Sender(const ContactRef& contact, LTPUDPConvergenceLayer * parent);
        ...
        virtual void           Send (std::string * send_data, SPtr_LTPTimer& timer, bool back) override;
        virtual void           Send_Low_Priority(std::string * send_data, SPtr_LTPTimer& timer) override;
        ...
        virtual uint64_t       Ltp_Queued_Bytes_Quota() override;
    ...
    class SenderRef : public CLInfo, 
                      public LTPCLSenderIF
    {
    public:
        /**
         * Constructor to make a new shared pointer to a Sender instance
         */
        SenderRef(const ContactRef& contact, LTPUDPConvergenceLayer *ltpcl);

        /**
         * Constructor that adss a share to an existinsg shared_ptr ref
         */
        SenderRef(const SenderRef& sender_ref);
        SenderRef(SenderRef* sref_ptr);
         ...
         */
        SPtr_Sender sptr() { return sender_instance_; }

        // access to Sender methods
        virtual bool init(Params* params, in_addr_t addr, u_int16_t port);
        virtual void start();
        virtual void shutdown();
        virtual bool reconfigure(Params* new_params);
        virtual void dump_link(oasys::StringBuffer* buf);

        /**
         * virtuals from CLInfo in ConvergenceLayer.h
         */
        virtual size_t get_incoming_list_size() override;
        virtual void get_cla_stats(oasys::StringBuffer& buf) override;
        ...
        virtual void           Set_Ready_For_Bundles(bool input_flag) override;
        virtual void           Send (std::string * send_data, SPtr_LTPTimer& timer, bool back) override;
        virtual void           Send_Low_Priority(std::string * send_data, SPtr_LTPTimer& timer) override;
        virtual uint32_t       Retran_Intvl() override;
        virtual uint32_t       Retran_Retries() override;
