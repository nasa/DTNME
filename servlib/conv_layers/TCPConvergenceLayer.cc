...
/****
    TODO: How to configure these items for an Interface???

    p.addopt(new oasys::BoolOpt("reliable", &params->reliable_));
    p.addopt(new oasys::UInt64Opt("mtu", &params->mtu_));
    p.addopt(new oasys::UIntOpt("idle_close_time",
                                &params->idle_close_time_));
    p.addopt(new oasys::UIntOpt("potential_downtime",
                                &params->potential_downtime_));
    p.addopt(new oasys::BoolOpt("prevhop_hdr", &params->prevhop_hdr_));
    p.addopt(new oasys::UIntOpt("cost", &params->cost_));
    p.addopt(new oasys::BoolOpt("qlimit_enabled", &params->qlimit_enabled_));
    p.addopt(new oasys::SizeOpt("qlimit_bundles_high",
                                &params->qlimit_bundles_high_));
    p.addopt(new oasys::SizeOpt("qlimit_bytes_high",
                                &params->qlimit_bytes_high_));
    p.addopt(new oasys::SizeOpt("qlimit_bundles_low",
                                &params->qlimit_bundles_low_));
    p.addopt(new oasys::SizeOpt("qlimit_bytes_low",
                                &params->qlimit_bytes_low_));
...
TCPConvergenceLayer::Connection3::Connection3(TCPConvergenceLayer* cl,
    ...
    : StreamConvergenceLayer::Connection("TCPConvergenceLayer::Connection3",
                                         ...
                                         true /* call connect() */)
{
    logpathf("%s/conn3/%p", cl->logpath(), this);
...
TCPConvergenceLayer::Connection4::Connection4(TCPConvergenceLayer* cl,
    ...
    : StreamConvergenceLayer::Connection("TCPConvergenceLayer::Connection4",
                                         ...
                                         false /* call accept() */)
{
    logpathf("%s/conn4/%p", cl->logpath(), this);
...
TCPConvergenceLayer::Connection4::handle_contact_initiation()
{
 ...
 tooshort:
    ...
    if (!active_connector_) {
        initiate_contact();  // send our header info before we disconnect or continue
    }
...
TCPConvergenceLayer::Connection4::handle_session_negotiation()
{
     ...
     */
    EndpointID peer_eid;
    if (! peer_eid.assign(node_id_ptr, node_id_len)) {
        log_err("protocol error: invalid endpoint id '%s' (len %llu)",
                peer_eid.c_str(), U64FMT(node_id_len));
        break_contact(ContactEvent::CL_ERROR);
        return true;
    }
    ...
    if (!find_contact(peer_eid)) {
        ...
        //dzdebug log_debug("TCPConvergenceLayer::Connection4::"
        log_err("TCPConvergenceLayer::Connection4::"
    ...
    logpathf("%s/conn4/%s", cl_->logpath(), peer_eid.c_str());
...
TCPConvergenceLayer::Connection4::handle_refuse_bundle(uint8_t flags)
{
    ...
    if (current_inflight_ && (current_inflight_->transfer_id_ == transfer_id)) {
        inflight = current_inflight_;
        found = true;
    } else {
    ...
    if (ctr == 0) {
        log_warn("TCPCLv4 - Current inflight bundle refused (transfer id: %" PRIu64 " - %s): *%p", 
                 transfer_id, tcpv4_refusal_reason_to_str(reason), inflight->bundle_.object());
    } else {
...
TCPConvergenceLayer::Connection4::process_data()
{
    ...
    //dzdebug
    //log_always("%s - entered - recvbuf: size= %zu fullbytes= %zu tailbytes= %zu  recv_segment_todo= %zu",
    ...
    while (recvbuf_.fullbytes() != 0) {
        ...
        case TCPV4_MSG_TYPE_XFER_SEGMENT:
            ...
            if (delay_reads_to_free_some_storage_) {
                ok = false;  // stop processing of the recvbuf
            }
            break;
