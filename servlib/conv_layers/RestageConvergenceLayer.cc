...
class oasys::SpinLock RestageConvergenceLayer::scandir_lock_;
class oasys::SpinLock RestageConvergenceLayer::ExternalStorageController::Emailer::sendmail_pipe_lock_;
...
RestageConvergenceLayer::Params::serialize(oasys::SerializeAction *a)
{
    ...
    a->process("max_quota_overage", &max_quota_overage_);
    a->process("part_of_pool", &part_of_pool_);
    a->process("email_enabled", &email_enabled_);
    a->process("min_vol_avail", &min_vol_available_);
    a->process("min_quota_avail", &min_quota_available_);
    a->process("field_separator", &field_separator_);
    a->process("eid_field_separator", &eid_field_separator_);
                                  ...
                                  const char** invalidp)
{
    ...
    p.addopt(new oasys::SizeOpt("max_quota_overage", &params->max_quota_overage_));
    ...
    p.addopt(new oasys::SizeOpt("min_vol_avail", &params->min_vol_available_));
    ...
    if (count == -1) {
        ...
        if (params->email_enabled_) {
            if (params->sptr_emails_->email_list_.size() > 0) {
                ...
                log_err("No add_email specified for alerts - disabling email");
                params->email_enabled_ = false;
            }
        }
    }
                                         ...
                                         AttributeVector& av_params)
{
    ...
    for (iter = av_params.begin(); iter != av_params.end(); iter++) {
        ...
        } else if (iter->name().compare("max_quota_overage") == 0) {
            if (iter->type() == NamedAttribute::ATTRIBUTE_TYPE_UNSIGNED_INTEGER) {
                sptr_params->max_quota_overage_ = iter->u_int_val();
            } else if (iter->type() == NamedAttribute::ATTRIBUTE_TYPE_INTEGER) {
                sptr_params->max_quota_overage_ = iter->int_val();
            } else if (iter->type() == NamedAttribute::ATTRIBUTE_TYPE_UNSIGNED_INT64) {
                sptr_params->max_quota_overage_ = iter->uint64_val();
            } else if (iter->type() == NamedAttribute::ATTRIBUTE_TYPE_INT64) {
                sptr_params->max_quota_overage_ = iter->int64_val();
            }

        } else if (iter->name().compare("min_vol_avail") == 0) {
            if (iter->type() == NamedAttribute::ATTRIBUTE_TYPE_UNSIGNED_INTEGER) {
                sptr_params->min_vol_available_ = iter->u_int_val();
            } else if (iter->type() == NamedAttribute::ATTRIBUTE_TYPE_INTEGER) {
                sptr_params->min_vol_available_ = iter->int_val();
            } else if (iter->type() == NamedAttribute::ATTRIBUTE_TYPE_UNSIGNED_INT64) {
                sptr_params->min_vol_available_ = iter->uint64_val();
            } else if (iter->type() == NamedAttribute::ATTRIBUTE_TYPE_INT64) {
                sptr_params->min_vol_available_ = iter->int64_val();
            }
...
RestageConvergenceLayer::list_link_opts(oasys::StringBuffer& buf)
{
    ...
    buf.append("    days_retention <U64>               - Maximum number of days to store bundles for retrieval (default: 30)\n");
    ...
    buf.append("    min_vol_avail <Size>               - Declare FULL_DISK when available space less than <Size> (default = 100MB)\n");

    buf.append("    disk_quota <Size>                  - Max disk space to use for storage (default: 0 = no limit other than disk space)\n");
    buf.append("    max_quota_overage <Size>           - Max allowed over quota for writing a bundle to disk (default: 1MB)\n");
    buf.append("                                            (allows full utilization of the quota so it can be declared FULL_QUOTA)\n");
    ...
    buf.append("    field_separator <Char>             - Character to use in filename to separate fields (default: _)\n");
    buf.append("    eid_field_separator <Char>         - Character to use in EID to separate components (default: -)\n");
    ...
    buf.append("     * retain bundles for 30 days regardless of their expiration time since expire_bundles defaults to false;\n");
...
//----------------------------------------------------------------------
RestageConvergenceLayer::ExternalStorageController::ExternalStorageController(const ContactRef& contact, 
...
{
    if (sptr_clstatus_->cl_state() != RESTAGE_CL_STATE_SHUTDOWN) {
        shutdown();
    }
}
...
RestageConvergenceLayer::ExternalStorageController::shutdown()
{
    ...
    update_restage_cl_state(RESTAGE_CL_STATE_SHUTDOWN);

    // inform the BARD that this instance is exiting
    std::string tmp_link_name = sptr_clstatus_->restage_link_name();
    sptr_bard_->unregister_restage_cl(tmp_link_name);
...
RestageConvergenceLayer::ExternalStorageController::init(SPtr_RestageCLParams sptr_params, const std::string& link_name)
{
    ...
    oasys::ScopeLock scoplok(sptr_clstatus_->lock(), __func__);
    sptr_clstatus_->set_restage_link_name(link_name);

    // copy paramter info to the status info for sharing with the BARD
    sptr_clstatus_->set_storage_path(sptr_params_->storage_path_);
    sptr_clstatus_->set_mount_point(sptr_params_->mount_point_);
    sptr_clstatus_->set_disk_quota(sptr_params_->disk_quota_);
    sptr_clstatus_->set_part_of_pool(sptr_params_->part_of_pool_);
    sptr_clstatus_->set_email_enabled(sptr_params_->email_enabled_);

    sptr_clstatus_->set_days_retention(sptr_params_->days_retention_);
    sptr_clstatus_->set_expire_bundles(sptr_params_->expire_bundles_);
    sptr_clstatus_->set_ttl_override(sptr_params_->ttl_override_);
    sptr_clstatus_->set_auto_reload_interval(sptr_params_->auto_reload_interval_);
...
RestageConvergenceLayer::ExternalStorageController::reconfigure(SPtr_RestageCLParams sptr_params)
{
    ...
    sptr_clstatus_->set_disk_num_files(grand_total_num_files_);
    sptr_clstatus_->set_disk_quota_in_use(grand_total_num_bytes_);

    // quotas might have changed so check the online state
    update_volume_online_state();
}
...
RestageConvergenceLayer::ExternalStorageController::dump_link(oasys::StringBuffer* buf)
{
    ...
    buf->appendf("storage_path: %s\n", sptr_clstatus_->storage_path().c_str());
    if (sptr_clstatus_->mount_point()) {
        if (sptr_clstatus_->mount_pt_validated()) {
            buf->appendf("mount_point: true (validated as %s)\n", sptr_clstatus_->validated_mount_pt().c_str());
        } else {
    ...
    buf->appendf("part_of_pool: %s\n", sptr_params_->part_of_pool_ ? "true" : "false");
    ...
    buf->appendf("days_retention: %" PRIu64 "\n", sptr_clstatus_->days_retention());
    buf->appendf("expire_bundles: %s\n", sptr_clstatus_->expire_bundles() ? "true" : "false");




    buf->appendf("ttl_override: %" PRIu64 "\n", sptr_clstatus_->ttl_override());
    buf->appendf("auto_reload_interval: %" PRIu64 "\n", sptr_clstatus_->auto_reload_interval());
    if (sptr_clstatus_->auto_reload_interval() > 0) {
        ...
        size_t time_to_next = sptr_clstatus_->auto_reload_interval() - 
    ...
    size_t tmp_val = sptr_clstatus_->vol_total_space();
    buf->appendf("volume size: %zu (%s)\n", 
                 tmp_val, FORMAT_WITH_MAG(tmp_val).c_str());

    tmp_val = sptr_clstatus_->vol_block_size();
    buf->appendf("volume block size: %zu\n", tmp_val);

    tmp_val = sptr_clstatus_->vol_space_available();
    buf->appendf("volume available: %zu (%s)\n", tmp_val, FORMAT_WITH_MAG(tmp_val).c_str());

    tmp_val = sptr_params_->min_vol_available_;
    buf->appendf("min_vol_avail: %zu (%s)  (declare FULL_DISK when available is less than this)\n", 
                 tmp_val, FORMAT_WITH_MAG(tmp_val).c_str());

    tmp_val = sptr_clstatus_->disk_quota();
    buf->appendf("disk_quota: %zu (%s)\n", tmp_val, FORMAT_WITH_MAG(tmp_val).c_str());

    tmp_val = sptr_clstatus_->disk_quota_in_use();
    buf->appendf("quota in use: %zu (%s)\n", tmp_val, FORMAT_WITH_MAG(tmp_val).c_str());

    tmp_val = sptr_params_->max_quota_overage_;
    buf->appendf("max_quota_overage: %zu (%s)  (allows final bundle to go over a bit to fully use quota)\n", 
                 tmp_val, FORMAT_WITH_MAG(tmp_val).c_str());

    tmp_val = sptr_params_->min_quota_available_;
    buf->appendf("min_quota_avail: %zu (%s)  (available quota needed to declare ONLINE after a FULL_QUOTA)\n", 
                 tmp_val, FORMAT_WITH_MAG(tmp_val).c_str());

    const char* state_str = sptr_clstatus_->cl_state_str();
    buf->appendf("State: %s\n", state_str);
...
RestageConvergenceLayer::ExternalStorageController::run()
{
    ...
    // whether to delete restaged bundles while initializing
    storage_tidy_ = BundleDaemon::params_.storage_tidy_;

    validate_external_storage();
    update_volume_usage_statitics();

    log_always("RestageCL initialization took %" PRIu64 " ms", init_timer.elapsed_ms());

    qptr_restager_ = std::unique_ptr<Restager>(new Restager(sptr_esc_self_, sptr_clstatus_->restage_link_name(),
                                                            contact_, sptr_params_));
    qptr_restager_->start();

    qptr_reloader_ = std::unique_ptr<Reloader>(new Reloader(sptr_clstatus_->restage_link_name(),
                                                            sptr_esc_self_, sptr_bard_));
    qptr_reloader_->start();

    oasys::Time error_state_timer;
    oasys::Time garbage_collection_timer;
    oasys::Time monitor_volume_timer;

    uint64_t one_minute_in_ms = 60 * 1000;
    uint64_t five_minutes_in_ms = 5 * one_minute_in_ms;
    uint64_t one_hour_in_ms = 60 * one_minute_in_ms;

    error_state_timer.get_time();
    auto_reload_timer_.get_time();
    garbage_collection_timer.get_time();
    monitor_volume_timer.get_time();

    while (!should_stop()) {
        usleep(100000);

        if (sptr_clstatus_->cl_state() == RESTAGE_CL_STATE_ERROR) {
        ...
        if (monitor_volume_timer.elapsed_ms() >= one_minute_in_ms) {
            update_volume_usage_statitics();
            monitor_volume_timer.get_time();
        }
...
RestageConvergenceLayer::ExternalStorageController::rescan_external_storage()
{
    ...
    do {
        oasys::ScopeLock clstatus_scoplok(sptr_clstatus_->lock(), __func__);

        sptr_clstatus_->set_disk_num_files(0);
        sptr_clstatus_->set_disk_quota_in_use(0);
...
RestageConvergenceLayer::ExternalStorageController::validate_external_storage()
{
    ...
    if (sptr_clstatus_->mount_point()) {
        std::string tmp_path = sptr_clstatus_->storage_path();
        std::string tmp_validated_mnt;
        bool tmp_validated = validate_mount_point(tmp_path, tmp_validated_mnt);

        sptr_clstatus_->set_validated_mount_pt(tmp_validated_mnt);
        sptr_clstatus_->set_mount_pt_validated(tmp_validated);


    } else {
        // flag the location as validated since it is not a mount point we need to check
        sptr_clstatus_->set_mount_pt_validated(true);
        sptr_clstatus_->set_validated_mount_pt("/");
    }
    ...
    if (sptr_clstatus_->mount_pt_validated()) {
        std::string tmp_path = sptr_clstatus_->storage_path();
        bool tmp_bool = create_storage_path(tmp_path);
        sptr_clstatus_->set_storage_path_exists(tmp_bool);

        if (sptr_clstatus_->storage_path_exists()) {
            is_online = true;
        }
    }
...
//----------------------------------------------------------------------
bool
RestageConvergenceLayer::ExternalStorageController::update_volume_usage_statitics()
{
    bool result = false;
    ...
    errno = 0;
    int retval = statvfs(sptr_clstatus_->storage_path().c_str(), &vol_stats);

    while ((retval != 0) && (errno == EINTR)) {
        errno = 0;
        retval = statvfs(sptr_clstatus_->storage_path().c_str(), &vol_stats);
    }

    if (retval != 0) {
        switch (errno) {
            case EACCES:
                    log_err("%s: EACCESS - Insufficient permission to search path: %s", __func__, sptr_clstatus_->storage_path().c_str());
                    break;
            case EFAULT:
                    log_err("%s: EFAULT - Invalid address supplied to statvfs. path: %s", __func__, sptr_clstatus_->storage_path().c_str());
                    break;
            case EIO:
                    log_err("%s: EIO - I/O error while reading from file system. path: %s", __func__, sptr_clstatus_->storage_path().c_str());
                    break;
            case ELOOP:
                    log_err("%s: ELOOP - Too many symbolic links in path: %s", __func__, sptr_clstatus_->storage_path().c_str());
                    break;
            case ENAMETOOLONG:
                    log_err("%s: ENAMETOOLONG - Path is too long: %s", __func__, sptr_clstatus_->storage_path().c_str());
                    break;
            case ENOENT:
                    log_err("%s: ENOENT - File does not exist. path: %s", __func__, sptr_clstatus_->storage_path().c_str());
                    break;
            case ENOMEM:
                    log_err("%s: ENOMEM - Insufficient kernel memory. path: %s", __func__, sptr_clstatus_->storage_path().c_str());
                    break;
            case EOVERFLOW:
                    log_err("%s: EOVERFLOW - Some values were too large to return by statvfs. path: %s", __func__, sptr_clstatus_->storage_path().c_str());
                    break;

            default:
                    char buf[128];
                    strerror_r(errno, buf, sizeof(buf));
                    log_err("%s: Unexpected error: %s; path: %s", __func__, buf, sptr_clstatus_->storage_path().c_str());
                    break;
        ...
        sptr_clstatus_->set_cl_state(RESTAGE_CL_STATE_ERROR);
        sptr_clstatus_->set_vol_total_space(0);
        sptr_clstatus_->set_vol_space_available(0);
    } else {
        result = true;

        size_t old_vol_avail = sptr_clstatus_->vol_space_available();
        size_t new_vol_avail = (size_t) vol_stats.f_bsize * (size_t) vol_stats.f_bavail;

        if (new_vol_avail < old_vol_avail) {
            log_info("%s: Setting Volume Space Available to (%zu - %s) which is lower than calculated (%zu)",
                     __func__, new_vol_avail, FORMAT_WITH_MAG(new_vol_avail).c_str(), old_vol_avail);
        } else if (new_vol_avail > old_vol_avail) {
            log_info("%s: Setting Volume Space Available to (%zu - %s) which is higher than calculated (%zu)",
                     __func__, new_vol_avail, FORMAT_WITH_MAG(new_vol_avail).c_str(), old_vol_avail);
        }

        sptr_clstatus_->set_vol_total_space((size_t) vol_stats.f_frsize * (size_t) vol_stats.f_blocks);
        sptr_clstatus_->set_vol_space_available(new_vol_avail);
        sptr_clstatus_->set_vol_block_size((size_t) vol_stats.f_bsize);
...
//----------------------------------------------------------------------
void
RestageConvergenceLayer::ExternalStorageController::update_volume_online_state()
{
    oasys::ScopeLock clstatus_scoplok(sptr_clstatus_->lock(), __func__);

    if (sptr_clstatus_->cl_state() == RESTAGE_CL_STATE_ERROR) {
        // do not override the error state
    } else {
        if (sptr_clstatus_->vol_space_available() < sptr_params_->min_vol_available_) {
            // disk full state always overrides quota full state
            sptr_clstatus_->set_disk_space_full(true);
            
            update_restage_cl_state(RESTAGE_CL_STATE_FULL_DISK);
        } else {
            sptr_clstatus_->set_disk_space_full(false);

            size_t disk_quota = sptr_params_->disk_quota_;

            if (disk_quota > 0) {
                size_t quota_in_use = sptr_clstatus_->disk_quota_in_use();

                if (sptr_clstatus_->disk_quota_full()) {
                    if (quota_in_use < disk_quota) {
                        size_t quota_avail = disk_quota - quota_in_use;

                        if (quota_avail > sptr_params_->min_quota_available_) {
                            sptr_clstatus_->set_disk_quota_full(false);

                            update_restage_cl_state(RESTAGE_CL_STATE_ONLINE);
                        } else {
                            update_restage_cl_state(RESTAGE_CL_STATE_FULL_QUOTA);
                        }
                    } else {
                        update_restage_cl_state(RESTAGE_CL_STATE_FULL_QUOTA);
                    }
                    ...
                    if (quota_in_use >= disk_quota) {
                        sptr_clstatus_->set_disk_quota_full(true);
...
{
    full_path = sptr_clstatus_->storage_path();
    full_path.append("/");
...
//----------------------------------------------------------------------
void
RestageConvergenceLayer::ExternalStorageController::eid_to_tokens(const SPtr_EID& sptr_eid, std::string& scheme_token,
                                                                  std::string& nodename, std::string& service)
{
    ...
    if (sptr_eid->is_ipn_scheme()) {
        ...
        service = std::to_string(sptr_eid->service_num());

    } else if (sptr_eid->is_dtn_scheme()) {
        scheme_token = TOKEN_DTN;
        nodename = sptr_eid->uri().authority();
        service = sptr_eid->uri().path();
...
RestageConvergenceLayer::ExternalStorageController::scan_external_storage()
{
    ...
    std::string path_base = sptr_clstatus_->storage_path() + "/";
    std::string dirpath;
    ...
    scandir_lock_.lock(__func__);

    log_always("scanning external storage: %s", sptr_clstatus_->storage_path().c_str());

    scandir64_dirpath = path_base;

    int dcount = scandir64(sptr_clstatus_->storage_path().c_str(), &dirfiles, scandir64_filter_dirs, nullptr);

    scandir_lock_.unlock();
    ...
    for (int ix=0; ix<dcount; ++ix) {
        ...
        if (parse_dirname(dirname, quota_type, scheme, nodename, node_number)) {
            ...
            if (storage_tidy_) {
                // attempt to delete the directory if tidying storage
                rmdir(dirpath.c_str());
            }
    ...
    storage_tidy_ = false; // only invoke tidy option on first scan
                                                                              ...
                                                                              bard_quota_type_t quota_type)
{
    ...
    std::string delete_fullpath;

    std::string stat_fullpath;
    std::string filename;
    SPtr_BundleFileDesc sptr_bfd;

    size_t found = 0;
    size_t delete_count = 0;
    size_t delete_error_count = 0;
    ...
    log_info("scanning storage subdirectory: %s", dirpath.c_str());
    ...
    for (int ix=0; ix<dcount; ++ix) {
        ...
        if (parse_filename(filename, sptr_bfd)) {
            ...
            if (file_iter == sptr_bfd_map->end()) {
                ...
                if (storage_tidy_) {
                    delete_fullpath = dirpath + "/";
                    delete_fullpath.append(filename);

                    // delete the file
                    if (0 != unlink(delete_fullpath.c_str())) {
                        log_err("Error deleting restaged bundle file during storage tidy: %s - %s",
                        delete_fullpath.c_str(), strerror(errno));
                        ++delete_error_count;
                    } else {
                        ++delete_count;
                    }
                ...
                stat_fullpath = dirpath + "/";
                stat_fullpath.append(filename);
                if (0 == stat(stat_fullpath.c_str(), &stat_buf)) {
                    ...
                    sptr_bfd->file_size_  = sptr_bfd->payload_length_;
                    sptr_bfd->disk_usage_ = sptr_bfd->payload_length_;          // add some slop??
    ...
    if (storage_tidy_) {
        log_always("scanned subdirectory (tidy): %s  #files: %d  #bundles deleted: %zu  errors: %zu", 
                   dirname.c_str(), dcount, delete_count, delete_error_count);
    } else {
        log_always("scanned subdirectory: %s  #files: %d  #bundles: %zu", 
                                                                      ...
                                                                      const BundleFileDesc* bfd_ptr)
{
    ...
    // subtract from volume available space
    sptr_clstatus_->dec_vol_space_available(bfd_ptr->disk_usage_);
    update_volume_online_state();
}
                                                                          ...
                                                                          const SPtr_EID& sptr_eid)
{
    ...
    if (sptr_eid->is_ipn_scheme()) {
        ...
        bfd_ptr->src_eid_node_number_ = sptr_eid->node_num();
        bfd_ptr->src_eid_nodename_ = std::to_string(sptr_eid->node_num());
        bfd_ptr->src_eid_service_ = std::to_string(sptr_eid->service_num());

    } else if (sptr_eid->is_dtn_scheme()) {
        bfd_ptr->src_eid_scheme_ = BARD_QUOTA_NAMING_SCHEME_DTN;
        bfd_ptr->src_eid_nodename_ = sptr_eid->uri().authority();
        bfd_ptr->src_eid_service_ = sptr_eid->uri().path();
                                                                          ...
                                                                          const SPtr_EID& sptr_eid)
{
    ...
    if (sptr_eid->is_ipn_scheme()) {
        ...
        bfd_ptr->dst_eid_node_number_ = sptr_eid->node_num();
        bfd_ptr->dst_eid_nodename_ = std::to_string(sptr_eid->node_num());
        bfd_ptr->dst_eid_service_ = std::to_string(sptr_eid->service_num());

    } else if (sptr_eid->is_dtn_scheme()) {
        bfd_ptr->dst_eid_scheme_ = BARD_QUOTA_NAMING_SCHEME_DTN;
        bfd_ptr->dst_eid_nodename_ = sptr_eid->uri().authority();
        bfd_ptr->dst_eid_service_ = sptr_eid->uri().path();
                                                                    ...
                                                                    size_t file_size, size_t disk_usage)
{
    ...
    if (file_iter == sptr_bfd_map->end()) {
        ...
        log_info("%s: ignoring duplicate file in ESC stats: %s", __func__, filename.c_str());
        // must be rescanning the directory - just ignore
...
RestageConvergenceLayer::ExternalStorageController::update_restage_cl_state(restage_cl_states_t new_state)
{
    ...
    if (new_state != sptr_clstatus_->cl_state()) {
        ...
        case RESTAGE_CL_STATE_ONLINE:
            log_as_error = false;
            sptr_clstatus_->set_cl_state(RESTAGE_CL_STATE_ONLINE);
            break;

        case RESTAGE_CL_STATE_LOW:
            // not currently in use
            log_as_error = false;
            sptr_clstatus_->set_cl_state(RESTAGE_CL_STATE_LOW);
            break;

        case RESTAGE_CL_STATE_HIGH:
            // not currently in use
            log_as_error = false;
            sptr_clstatus_->set_cl_state(RESTAGE_CL_STATE_HIGH);
            break;

        case RESTAGE_CL_STATE_FULL_QUOTA:
            log_as_error = false;
            sptr_clstatus_->set_cl_state(RESTAGE_CL_STATE_FULL_QUOTA);
            break;

        case RESTAGE_CL_STATE_FULL_DISK:
            sptr_clstatus_->set_cl_state(RESTAGE_CL_STATE_FULL_DISK);
            break;
        ...
        case RESTAGE_CL_STATE_SHUTDOWN:
            sptr_clstatus_->set_cl_state(RESTAGE_CL_STATE_SHUTDOWN);
            break;

        default:
            sptr_clstatus_->set_cl_state(RESTAGE_CL_STATE_UNDEFINED);
            break;
        }
        ...
        if (log_as_error) {
            ...
            log_info("Restage CL State transitioned to: %s", 
                     restage_cl_states_to_str(new_state));
        }
    }
...
RestageConvergenceLayer::ExternalStorageController::send_email_notifications(restage_cl_states_t new_state)
{
    ...
    if (sptr_params_->email_enabled_) {
        ...
        std::string message = "DTNME Restage CL [";
        message += sptr_clstatus_->restage_link_name();
        message += "] transitioned to State: ";
        message += restage_cl_states_to_str(new_state);

        std::string message2 = "";

        if (sptr_params_->sptr_emails_->email_list_.size() > 0) {
            // kick off a new thread to send the emails
            Emailer* emailer = new Emailer(message, message2, sptr_params_->sptr_emails_,
                                           false, sptr_clstatus_->restage_link_name());
            emailer->start();
        }
    }
}
...
//----------------------------------------------------------------------
void
RestageConvergenceLayer::ExternalStorageController::send_email_notifications(std::string& message, std::string& message2)
{
    //
    if (sptr_params_->email_enabled_) {
        oasys::ScopeLock scoplok(&sptr_params_->sptr_emails_->lock_, __func__);
...
RestageConvergenceLayer::ExternalStorageController::disk_quota_is_available(ssize_t space_needed)
{
    ...
    if (sptr_clstatus_->disk_space_full()) {
        // no room on disk
        result = false;
    } else if (sptr_clstatus_->disk_quota_full()) {
        // no room in quota
        result = false;
    } else {
        if (sptr_params_->disk_quota_ > 0) {
            ...
            if (new_total > (sptr_params_->disk_quota_ + sptr_params_->max_quota_overage_)) {
                result = false;

                // TODO: anything to do here???

                // large bundle may be over quota but small bundle may not be????
//dzdebug                // set the state to indicate the drive is full
//dzdebug                update_restage_cl_state(RESTAGE_CL_STATE_FULL);
            }
        }
    }
                                                                        ...
                                                                        const BundleFileDesc* bfd_ptr)
{
    ...
    if (stats_error) {
        log_err("%s - directory level statistics are out of sync for %s",
                  sptr_clstatus_->restage_link_name().c_str(), dirname.c_str());
    ...
    if (stats_error) {
        log_err("%s - grand total statistics are out of sync", 
                sptr_clstatus_->restage_link_name().c_str());
        // trigger a rescan??
    }
    ...
    // add to volume available space
    sptr_clstatus_->inc_vol_space_available(bfd_ptr->disk_usage_);
    update_volume_online_state();
}
...
RestageConvergenceLayer::ExternalStorageController::reload_all(size_t new_expiration)
{
    ...
    while (dir_iter != restage_dir_map_.end()) {
        ...
        dir_path = sptr_clstatus_->storage_path();
        dir_path.append("/");
        dir_path.append(dirname);

        sptr_bfd_map = dir_iter->second;

        // use the CL default TTL override if none specified
        if ( new_expiration == 0 )
        {
          new_expiration = sptr_params_->ttl_override_;
        }
...
RestageConvergenceLayer::ExternalStorageController::Restager::check_for_bundle()
{
    ...
    if (len > 0) {
        ...
        BundleRestagedEvent* event_to_post;
        event_to_post = new BundleRestagedEvent(bref.object(), contact_, link_, 
                                                len, true, false);
        SPtr_BundleEvent sptr_event_to_post(event_to_post);
        BundleDaemon::post(sptr_event_to_post);
    } else {
...
RestageConvergenceLayer::ExternalStorageController::Restager::process_bundle(BundleRef& bref)
{
                ...
                if (buf_size > 0) {
                    ...
                    if (n <= 0) {
                        ...
                        if (save_errno == ENOSPC) {
                            sptr_esc_->set_cl_state(RESTAGE_CL_STATE_FULL_DISK);
                        } else {
    ...
    if (error_writing_file) {
        ...
        // If state is Open then update 

        // let the Bundle Restaging Daemon know this bundle was restaged
        BundleDaemon::instance()->bard_bundle_restaged(bundle, disk_usage);
                                                                                   ...
                                                                                   SPtr_EID& sptr_new_dest_eid)
{
    ...
    if (sptr_new_dest_eid != nullptr) {
        quota_type = BARD_QUOTA_TYPE_DST;

        if (sptr_new_dest_eid->is_ipn_scheme()) {
            scheme = BARD_QUOTA_NAMING_SCHEME_IPN;
            nodename = std::to_string(sptr_new_dest_eid->node_num());

        } else if (sptr_new_dest_eid->is_imc_scheme()) {
            scheme = BARD_QUOTA_NAMING_SCHEME_IMC;
            nodename = std::to_string(sptr_new_dest_eid->node_num());

        } else {
            scheme = BARD_QUOTA_NAMING_SCHEME_DTN;
            nodename = sptr_new_dest_eid->ssp();
        }

    } else {
        quota_type = bfd_ptr->quota_type_;
...
RestageConvergenceLayer::ExternalStorageController::Reloader::process_reload(ReloadEvent* event)
{
    ...
    std::string new_dest_node_name;
    SPtr_EID   sptr_new_dest_eid;
    if (!event->new_dest_eid_.empty()) {
        sptr_new_dest_eid = BD_MAKE_EID(event->new_dest_eid_);

        if (!sptr_new_dest_eid->valid()) {
            log_err("RestageCL(%s) : reload attempt from %s with invalid override destination EID: %s", 
                    link_name_.c_str(), 
                    event->dir_path_.c_str(),
                    event->new_dest_eid_.c_str());
            // abort any further processing
            return;
        }

        log_info("RestageCL(%s) : Reloading bundles from %s with override Dest EID: %s and TTL: %zu",
                 link_name_.c_str(), 
                 event->dir_path_.c_str(),
                 event->new_dest_eid_.c_str(),
                 event->new_expiration_);
    } else {
        log_info("RestageCL(%s) : Reloading bundles from %s with override TTL: %zu",
                 link_name_.c_str(), 
                 event->dir_path_.c_str(),
                 event->new_expiration_);
    }
    ...
    while ((!should_stop()) && (iter != event->sptr_bfd_map_->end())) {
        ...
        if (!query_okay_to_reload(sptr_bfd.get(), sptr_new_dest_eid)) {
            // not okay - we are done
            log_info("RestageCL(%s) : Terminated reload from %s due to quota exceeded",
                     link_name_.c_str(), 
                     event->dir_path_.c_str());
            break;
        }
        ...
        if (err < 0)
        {
            ...
            if (bundle != nullptr) {
                ...
                if (sptr_new_dest_eid != nullptr) {
                    bundle->mutable_dest() = sptr_new_dest_eid;
                }
                ...
                bool accept_bundle = bdaemon->query_accept_bundle_based_on_quotas_internal_storage_only(bundle, 
                                                                                                        space_reserved,
                                                                                                        prev_reserved_space);
                ...
                if (!accept_bundle) {
                    ...
                    event_to_post = new BundleReceivedEvent(bundle, EVENTSRC_RESTAGE, 
                                                            sptr_bfd->file_size_, 
                                                            sptr_dummy_prevhop);
                    SPtr_BundleEvent sptr_event_to_post(event_to_post);
                    BundleDaemon::post(sptr_event_to_post);
    ...
    log_info("RestageCL(%s) : reloaded %zu bundles from %s", 
             link_name_.c_str(), num_reloaded, event->dirname_.c_str());
}
...
//----------------------------------------------------------------------
RestageConvergenceLayer::ExternalStorageController::Emailer::Emailer(std::string& message, 
                                                                     std::string& message2,
                                                                     SPtr_EmailNotifications sptr_emails,
                                                                     bool use_monospace_in_email,
                                                                     const std::string& link_name)
    : Logger("RestageCL:ESC:Emailer",
             "/dtn/cl/%s/emailer", link_name.c_str()),
      Thread("RestageCL::ESC::Emailer", Thread::DELETE_ON_EXIT),
      message_(message),
      message2_(message2),
      sptr_emails_(sptr_emails),
      use_monospace_in_email_(use_monospace_in_email)
{
}
...
RestageConvergenceLayer::ExternalStorageController::Emailer::run()
{
    ...
    oasys::ScopeLock scoplok_sendmail(&sendmail_pipe_lock_, __func__);

    oasys::ScopeLock scoplok_email_list(&sptr_emails_->lock_, __func__);

    EmailListIter iter = sptr_emails_->email_list_.begin();
    while (iter != sptr_emails_->email_list_.end()) {
        email_addr = iter->first;

        log_info("Send email from: %s  to: %s  - message = %s",
        ...
        if (mailpipe != nullptr) {
            ...
            fprintf(mailpipe, "Subject: DTNME Restage CL State Notification\n\n");

            if (use_monospace_in_email_) {
                // output MIME header to force message to use monospace --- did not work!!!
                fprintf(mailpipe, "MIME-Version: 1.0\n");
                fprintf(mailpipe, "Content-Type: text/html\n");
                fprintf(mailpipe, "Content-Disposition: inline\n");
                fprintf(mailpipe, "<html>\n");
                fprintf(mailpipe, "<body>\n");
                fprintf(mailpipe, "<pre style=\"font: monospace\">\n");
            }

            // output the message/email body
            fprintf(mailpipe, "%s\n",message_.c_str());
            ...
            if (use_monospace_in_email_) {
                // output MIME trailer to force message to use monospace
                fprintf(mailpipe, "</pre>\n");
                fprintf(mailpipe, "</body>\n");
                fprintf(mailpipe, "</html>\n");
            }
            ...
            log_err("Error invoking sendmail for DTNME Restage CL Notification");
        }
