...
ConnectionConvergenceLayer::close_contact(const ContactRef& contact)
{
    ...
    if (!conn->is_stopped()) {
        log_err("Connection not stopped after 2 seconds - tryig to force it to stop");
        conn->force_shutdown();
            ...
            (link->is_reliable() && acked_bytes == 0))
        {
            ...
            if (! link->del_from_inflight(inflight->bundle_))
            {
                ...
                if (inflight->bundle_->xmit_blocks()->find_blocks(link) == nullptr) {
                    SPtr_BlockInfoVec blocks = BundleProtocol::prepare_blocks(inflight->bundle_.object(), link);
                    BundleProtocol::generate_blocks(inflight->bundle_.object(), blocks, link);
                } else {
                    //dzdebug
    ...
    if (! conn->incoming_.empty()) {
        ...
        {  
            ...
            size_t header_block_length =
                BundleProtocol::payload_offset(incoming->bundle_.object(), incoming->bundle_->recv_blocks());
