...
CLConnection::contact_up()
{
    ...
    event_to_post = new ContactUpEvent(contact_);
...
//----------------------------------------------------------------------
bool
CLConnection::find_contact(const SPtr_EID& sptr_peer_eid)
{
    if (contact_ != NULL) {
    ...
    bool new_link = false;
    LinkRef link = cm->find_link_to(cl_, "", sptr_peer_eid,
                                    Link::OPPORTUNISTIC,
    ...
    if (link == NULL || (link != NULL && link->contact() != NULL)) {
        ...
        link = cm->new_opportunistic_link(cl_, nexthop_.c_str(), sptr_peer_eid);
        if (link == NULL) {
