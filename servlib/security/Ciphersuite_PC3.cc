...
//----------------------------------------------------------------------
int Ciphersuite_PC3::decrypt(std::string   security_dest,
							 const LocalBuffer&   input,      // RSA-encrypted Bundle Encryption Key (BEK)
							 LocalBuffer&   result)		      // decrypted BEK will go here
{
    return EVP_PK::decrypt(Ciphersuite::config->get_priv_key_dec(security_dest, cs_num()), input, result);
}

//----------------------------------------------------------------------
int Ciphersuite_PC3::encrypt(std::string       security_dest,
