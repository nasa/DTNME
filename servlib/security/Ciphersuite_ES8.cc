...
//----------------------------------------------------------------------
Ciphersuite_ES8::Ciphersuite_ES8()
{
}

//----------------------------------------------------------------------
u_int16_t
Ciphersuite_ES8::cs_num(void)
{
    return CSNUM_ES;
}


int Ciphersuite_ES8::decrypt(
                             ...
                             LocalBuffer&   result)	         // decrypted BEK will go here
{
    ...
    if(0 != ECDH::decrypt(result, priv_key_file.c_str(), enc_data)) {
        log_err_p(log, "Ciphersuite_ES8::decrypt: decryption failed");
        return -1;
    } else {
        log_debug_p(log, "Ciphersuite_ES8::decrypt decryption succeeded");
    }

    return 0;
}

int Ciphersuite_ES8::encrypt(
							 std::string       security_dest,
							 ...
							 LocalBuffer&       result)        // encrypted BEK will go here
{
	...
	return ECDH::encrypt(data, certfile.c_str(), result, 128);
}
